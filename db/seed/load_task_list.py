...
# repo root = .../Orange One ; the sheet lives in a sibling working dir.
XLSX = os.path.normpath(os.path.join(
    HERE, "..", "..", "..", "FETCH DAILY DATA", "MISC", "Task List (2).xlsx"))
...
def main():
    ...
    for r in range(3, ws.max_row + 1):
        ...
        task = clean(cells[2])
        freq = (cells[3] or "").strip()
        if not name or not category or not task or not freq:
            continue
        if name not in EMP:
            skipped += 1
            w(f"-- SKIPPED row {r}: unknown employee {name!r}")
            continue
