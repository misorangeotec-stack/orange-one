-- Stage B — add "WHEN" recurrence (generation part).
--
-- Replaces both generators so they fire "when" templates. A "when" template
-- fires every working day Mon–Sat (isodow 1..6), skipping only Sunday (isodow 7).
-- That is one day wider than 'daily' (which is Mon–Fri / isodow 1..5).
--
-- Dedup and the location-checklist copy are unchanged. Apply AFTER 0013 (which
-- adds the 'when' enum value + the tasks.not_applicable columns) — and apply 0013
-- in a separate run first (see its header). Purely additive: this is
-- create-or-replace of the two existing functions, no schema/data mutation.
--
-- Reversal: re-apply 0010 (restores both generators without the 'when' branch).

-- ---------------------------------------------------------------------------
-- 1. Bulk daily job (cron) — now also fires "when" templates (Mon–Sat).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. On-demand single-template generation — "when" added to the firing check.
