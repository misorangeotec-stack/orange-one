-- Stage B — QUARTERLY + monthly Nth-weekday recurrence (generation part).
--
-- Replaces both generators (built on 0014, which added the 'when' branch) to:
--   * Monthly: when monthly_weekday is set, fire on the monthly_nth occurrence
--     (1..5) of that weekday — e.g. nth=1, weekday=6 = "1st Saturday". Otherwise
--     keep the existing day-of-month logic (monthly_days; 32 = last day).
--   * Quarterly: fire 7 days before each quarter ends — i.e. on
--     (quarter_end - 7 days): Mar 24 / Jun 23 / Sep 23 / Dec 24.
--
-- Dedup and the location-checklist copy are unchanged. Apply AFTER 0015 (which
-- adds the 'quarterly' enum value + monthly_nth/monthly_weekday columns) — and
-- apply 0015 in a separate run first (see its header). Purely additive:
-- create-or-replace of the two existing functions, no schema/data mutation.
--
-- Reversal: re-apply 0014 (restores both generators without quarterly/Nth-weekday).

-- ---------------------------------------------------------------------------
-- 1. Bulk daily job (cron).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. On-demand single-template generation.
