...
_Last updated: Stage B — B1/B2/B3a/B3b done (whole app reads live data). B4 in progress (option B, careful live writes): create-task, Start/Complete/Revise (2/week limit), reschedule/shift-to-next-week, and @mention remarks + notification fan-out (via the new add_task_remark SECURITY DEFINER RPC) all live & verified end-to-end via throwaway records (counts return to 69/264/31 each time; no real data touched). First production DB change made (additive RPC, user-approved, in db/migrations/). Remaining writes (recurring CRUD, weekly-plan upsert, admin user/dept/access writes, mark-notifications-read) still read-only no-ops, rolling out next._
