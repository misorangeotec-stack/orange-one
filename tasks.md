...
- [ ] @mention remarks (+ notification fan-out)
...
_Last updated: Stage B — B1/B2/B3a/B3b done (whole app reads live data). B4 in progress (option B, careful live writes): create-task, Start/Complete/Revise (2/week limit), and reschedule/shift-to-next-week all live & verified end-to-end via throwaway records (counts return to 69/264/31 each time; no real data touched). Remaining writes (@mention remarks + notification fan-out, recurring CRUD, weekly-plan upsert, admin writes) still read-only no-ops, rolling out next._
