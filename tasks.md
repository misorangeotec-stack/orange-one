...
_Last updated: Stage B — B1/B2/B3a/B3b done (whole app reads live data). B4 in progress (option B, careful live writes): create-task + Start/Complete/Revise (with 2/week limit) all live & verified end-to-end via throwaway records (counts return to 69/264/31 each time; no real data touched). Fixed a modal state-persistence bug found during verification. Remaining writes (shift-to-next-week, @mention remarks + notification fan-out, recurring CRUD, weekly-plan upsert, admin writes) still read-only no-ops, rolling out next._
