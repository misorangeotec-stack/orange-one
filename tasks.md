...
- [x] **Add User / Delete User — code complete (awaiting deploy).** `supabase/functions/admin-users/` Edge Function (Deno): admin-only (re-checks the caller's role with the service role), `create` mints the auth user via the admin API → trigger makes the profile, then patches profile/role/reporting/modules; `delete` removes the auth user (cascades the profile). Frontend `core/platform/adminUserApi.ts` invokes it; store `addUser`/`deleteUser` wired async; UserForm + Users delete await with busy/error. **Gated off (`canAddUser`/`canDeleteUser`=false) until deployed** — deploy needs a Supabase management access token (`supabase login` / `SUPABASE_ACCESS_TOKEN`); steps in `supabase/functions/README.md`. After deploy: flip the two flags and verify with a throwaway user.
