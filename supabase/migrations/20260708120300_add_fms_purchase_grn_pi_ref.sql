-- ===========================================================================
-- Purchase FMS (procurement) — GRN carries an optional free-text PI reference.
--
...
alter table public.fms_purchase_grns add column if not exists pi_ref text;

drop function if exists public.fms_purchase_record_grn(uuid, jsonb, uuid, text, text, text, text, text);
...
begin
  ...
  insert into public.fms_purchase_grns (po_id, pi_id, pi_ref, gate_register_no, condition, note, received_by, photo_path, photo_name)
...
grant execute on function public.fms_purchase_record_grn(uuid, jsonb, uuid, text, text, text, text, text, text) to authenticated;
