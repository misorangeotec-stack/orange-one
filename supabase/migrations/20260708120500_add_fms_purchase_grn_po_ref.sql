-- ===========================================================================
-- Purchase FMS (procurement) — GRN is referenced by the PO, not the PI.
--
-- The gate records the goods receipt against the PO reference number; the vendor
-- PI number is only an extra remark. Adds a `po_ref` column (prefilled in the UI
-- from the PO's Tally/system number) alongside the existing optional `pi_ref`.
--
-- Additive / replace-only.
-- ===========================================================================

alter table public.fms_purchase_grns add column if not exists po_ref text;

-- Backfill from the PO's Tally number (else its system PO no.) so existing
-- receipts show the reference they were booked against.
update public.fms_purchase_grns g
   set po_ref = coalesce(p.tally_po_no, p.po_no)
  from public.fms_purchase_pos p
 where p.id = g.po_id
   and g.po_ref is null;

drop function if exists public.fms_purchase_record_grn(uuid, jsonb, uuid, text, text, text, text, text, text);
create or replace function public.fms_purchase_record_grn(
  ...
  p_photo_path text default '', p_photo_name text default '', p_pi_ref text default '', p_po_ref text default ''
)
...
begin
  ...
  insert into public.fms_purchase_grns (po_id, pi_id, po_ref, pi_ref, gate_register_no, condition, note, received_by, photo_path, photo_name)
  values (p_po_id, p_pi_id,
          coalesce(nullif(p_po_ref,''), (select coalesce(tally_po_no, po_no) from public.fms_purchase_pos where id = p_po_id)),
          nullif(p_pi_ref,''), nullif(p_gate_register_no,''), coalesce(nullif(p_condition,''),'good'), nullif(p_note,''), auth.uid(),
...
grant execute on function public.fms_purchase_record_grn(uuid, jsonb, uuid, text, text, text, text, text, text, text) to authenticated;
