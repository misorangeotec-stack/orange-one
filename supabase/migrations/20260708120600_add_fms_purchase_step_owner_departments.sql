-- ===========================================================================
-- Purchase FMS (procurement) — a step's owners may span SEVERAL departments.
--
-- fms_purchase_step_owners.department_id held a single department, but people
...
-- still comes solely from employee_ids (see fms_purchase_is_step_owner).
-- ===========================================================================

alter table public.fms_purchase_step_owners
  add column if not exists department_ids uuid[] not null default '{}';

-- Backfill the array from the legacy single department.
update public.fms_purchase_step_owners
   set department_ids = array[department_id]
 where department_id is not null
   and cardinality(department_ids) = 0;

comment on column public.fms_purchase_step_owners.department_ids is
