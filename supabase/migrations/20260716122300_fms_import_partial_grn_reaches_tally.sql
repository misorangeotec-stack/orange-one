-- ===========================================================================
-- Purchase FMS (import) — a PARTIAL GRN must reach Tally (and beyond).
--
...
with b as (
  select id, po_id, row_number() over (partition by po_id order by created_at, id) as rn
    from public.fms_import_tally_bookings
   where grn_id is null
),
g as (
  select gr.id, gr.po_id, row_number() over (partition by gr.po_id order by gr.created_at, gr.id) as rn
    from public.fms_import_grns gr
   where not exists (
     select 1 from public.fms_import_tally_bookings t where t.grn_id = gr.id
   )
)
update public.fms_import_tally_bookings t
   set grn_id = g.id
  from b
...
drop function if exists public.fms_import_book_tally(uuid, text, uuid, text, text, text);
...
begin
  ...
  select exists(select 1 from public.fms_import_grns where po_id = p_po_id) into v_has_grn;
  ...
  if p_grn_id is not null then
    select po_id into v_grn_po from public.fms_import_grns where id = p_grn_id;
    if v_grn_po is distinct from p_po_id then raise exception 'That GRN does not belong to this PO'; end if;
    if exists(select 1 from public.fms_import_tally_bookings where grn_id = p_grn_id) then
      raise exception 'That GRN is already booked in Tally';
    end if;
  end if;

  insert into public.fms_import_tally_bookings
...
grant execute on function public.fms_import_book_tally(uuid, text, uuid, text, text, text) to authenticated;
