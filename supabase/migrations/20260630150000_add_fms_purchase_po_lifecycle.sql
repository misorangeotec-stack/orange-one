-- Purchase FMS (procurement) — PO LIFECYCLE (Phase 4): Stages 5–10.
--
-- Tables:
--   fms_purchase_pis            — proforma invoices (one PO → many PIs; carries dispatch/follow-up)
--   fms_purchase_pi_items       — which PO lines/qty a PI covers
--   fms_purchase_grns           — goods receipts (one PI → many partial GRNs)
--   fms_purchase_grn_items      — per-item received qty (rolls into po_items.received_qty)
--   fms_purchase_tally_bookings — Stage 9 Tally PI no.
--   fms_purchase_payments       — advance + installments
--
-- RPCs (all SECURITY DEFINER, row-locked, authz by step owner or admin):
--   fms_purchase_share_po, fms_purchase_add_pi, fms_purchase_record_payment,
--   fms_purchase_record_followup, fms_purchase_record_grn, fms_purchase_book_tally
--
...
-- TABLES
-- ===========================================================================
create table if not exists public.fms_purchase_pis (
...
create index if not exists fms_purchase_pis_po_idx on public.fms_purchase_pis (po_id);
drop trigger if exists trg_fms_purchase_pis_updated on public.fms_purchase_pis;
create trigger trg_fms_purchase_pis_updated before update on public.fms_purchase_pis
  for each row execute function public.set_updated_at();

create table if not exists public.fms_purchase_pi_items (
  id         uuid primary key default gen_random_uuid(),
  pi_id      uuid not null references public.fms_purchase_pis on delete cascade,
  po_item_id uuid not null references public.fms_purchase_po_items on delete restrict,
  qty        numeric(14,3) not null check (qty > 0),
  created_at timestamptz not null default now()
);
create index if not exists fms_purchase_pi_items_pi_idx on public.fms_purchase_pi_items (pi_id);

create table if not exists public.fms_purchase_grns (
  id               uuid primary key default gen_random_uuid(),
  po_id            uuid not null references public.fms_purchase_pos on delete cascade,
  pi_id            uuid references public.fms_purchase_pis on delete set null,
...
create index if not exists fms_purchase_grns_po_idx on public.fms_purchase_grns (po_id);

create table if not exists public.fms_purchase_grn_items (
  id           uuid primary key default gen_random_uuid(),
...
create index if not exists fms_purchase_grn_items_grn_idx on public.fms_purchase_grn_items (grn_id);

create table if not exists public.fms_purchase_tally_bookings (
  id          uuid primary key default gen_random_uuid(),
  po_id       uuid not null references public.fms_purchase_pos on delete cascade,
  grn_id      uuid references public.fms_purchase_grns on delete set null,
  tally_pi_no text not null,
  booked_by   uuid references auth.users on delete set null,
  created_at  timestamptz not null default now()
);
create index if not exists fms_purchase_tally_po_idx on public.fms_purchase_tally_bookings (po_id);

create table if not exists public.fms_purchase_payments (
  id         uuid primary key default gen_random_uuid(),
  po_id      uuid not null references public.fms_purchase_pos on delete cascade,
  pi_id      uuid references public.fms_purchase_pis on delete set null,
...
create index if not exists fms_purchase_payments_po_idx on public.fms_purchase_payments (po_id);
...
begin
  foreach t in array array[
    'fms_purchase_pis','fms_purchase_pi_items','fms_purchase_grns','fms_purchase_grn_items',
    'fms_purchase_tally_bookings','fms_purchase_payments'
  ] loop
...
as $$
  select public.is_admin(p_uid)
    ...
    or public.fms_purchase_is_step_owner('final_payment', p_uid);
$$;

-- Stage 5a — mark PO shared.
create or replace function public.fms_purchase_share_po(p_po_id uuid)
...
grant execute on function public.fms_purchase_share_po(uuid) to authenticated;

-- Stage 5b — add a PI (with covered po-lines/qty).
drop function if exists public.fms_purchase_add_pi(uuid, text, text, numeric, date, jsonb);
...
begin
  ...
  insert into public.fms_purchase_pis (po_id, vendor_pi_no, payment_terms, pi_value, dispatch_date, created_by)
...
grant execute on function public.fms_purchase_add_pi(uuid, text, jsonb, text, numeric, date) to authenticated;

-- Stage 6 / 10 — record a payment (advance or installment).
drop function if exists public.fms_purchase_record_payment(uuid, uuid, text, numeric, date, text);
...
grant execute on function public.fms_purchase_record_followup(uuid, text, date, text, text, date) to authenticated;

-- Stage 8 — record a GRN (partial receipts allowed).
drop function if exists public.fms_purchase_record_grn(uuid, uuid, text, text, text, jsonb);
...
grant execute on function public.fms_purchase_record_grn(uuid, jsonb, uuid, text, text, text) to authenticated;

-- Stage 9 — Tally booking.
drop function if exists public.fms_purchase_book_tally(uuid, uuid, text);
...
begin
  ...
  insert into public.fms_purchase_tally_bookings (po_id, grn_id, tally_pi_no, booked_by)
  values (p_po_id, p_grn_id, p_tally_pi_no, auth.uid())
  returning id into v_id;
  perform public.fms_purchase_refresh_po(p_po_id);
  return v_id;
end $$;
grant execute on function public.fms_purchase_book_tally(uuid, text, uuid) to authenticated;
