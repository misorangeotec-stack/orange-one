...
--
-- Additive only: two new nullable columns on fms_import_pos (reusing the
-- existing private `fms-import-docs` bucket + policies from the PI-doc
...
drop function if exists public.fms_import_share_po(uuid);
drop function if exists public.fms_import_share_po(uuid, text, text);
...
grant execute on function public.fms_import_share_po(uuid, text, text, text, text) to authenticated;
