-- Purchase FMS (import) — FOLLOW-UP HISTORY + remarks.
--
...
create table if not exists public.fms_import_followups (
...
create index if not exists fms_import_followups_pi_idx on public.fms_import_followups (pi_id);

-- RLS — select-all; direct writes admin-only (the RPC is definer).
do $$
begin
  execute 'alter table public.fms_import_followups enable row level security';
  execute 'drop policy if exists fms_import_followups_select on public.fms_import_followups';
  execute 'create policy fms_import_followups_select on public.fms_import_followups for select to authenticated using (true)';
  execute 'drop policy if exists fms_import_followups_write_admin on public.fms_import_followups';
  execute 'create policy fms_import_followups_write_admin on public.fms_import_followups for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()))';
end $$;
...
drop function if exists public.fms_import_record_followup(uuid, text, date, text, text, date, text);
