...
-- Additive only: two new nullable columns on fms_purchase_pis, one private
-- storage bucket for the files, and two extra trailing params on the
-- fms_purchase_add_pi RPC so the uploaded file can be linked at insert time.
-- ===========================================================================

-- 1. Columns -----------------------------------------------------------------
alter table public.fms_purchase_pis
...
values ('fms-purchase-docs', 'fms-purchase-docs', false)
...
drop policy if exists "fms purchase docs read"   on storage.objects;
drop policy if exists "fms purchase docs insert" on storage.objects;
drop policy if exists "fms purchase docs update" on storage.objects;
drop policy if exists "fms purchase docs delete" on storage.objects;

create policy "fms purchase docs read" on storage.objects
  for select to authenticated using (bucket_id = 'fms-purchase-docs');
create policy "fms purchase docs insert" on storage.objects
  for insert to authenticated with check (bucket_id = 'fms-purchase-docs');
create policy "fms purchase docs update" on storage.objects
  for update to authenticated using (bucket_id = 'fms-purchase-docs') with check (bucket_id = 'fms-purchase-docs');
create policy "fms purchase docs delete" on storage.objects
  for delete to authenticated using (bucket_id = 'fms-purchase-docs');

-- 3. Recreate the add-PI RPC with the document params ------------------------
drop function if exists public.fms_purchase_add_pi(uuid, text, jsonb, text, numeric, date);
