-- KPI-1 · The score-by-week trend shows WHOLE weeks.
--
-- kpi_report's trend used to cut each ISO week to the period: in August, week 31 (27 Jul–2 Aug)
-- became just 1–2 Aug, and week 36 just 31 Aug. The user asked on 18-09-2026 for the proper week
-- instead — Monday to Sunday, even where it runs into July or September. So each trend bar is now
-- a whole week, and the facts read for it reach far enough to cover one.
--
-- NOTHING ELSE CHANGES. The period's own figures (given, done, on time, still due, last, next)
-- still come from its dates alone — August is still 1–31 August — and the rules are untouched.
-- Two edits to the function applied in 20261128121000_kpi1_facts.sql, generated from that file:
--   · the facts window: least(v_lf, v_tf) .. v_nt  →  also covers the first and last whole weeks;
--   · the weeks: clipped to the period  →  week_start .. week_start + 6.
-- `partial` stays in the output and is now always false.
--
-- Rollback: …_kpi1_report_full_weeks_rollback.sql restores the previous function verbatim.
