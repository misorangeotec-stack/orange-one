-- ===========================================================================
-- RM IMPORT PURCHASE FMS — QC INSPECTION (step 8) + the rejection branch.
--
...
-- purchase return.
--
-- QC is OPT-IN PER CATEGORY (`fms_import_categories.qc_required`), seeded true
-- for RAW MATERIAL only. Every other kind of purchase — Ink, Head, Machine, Spare
-- Parts — is untouched: Tally stays its last step and the PO closes exactly as it
-- does today.
--
-- Twin of 20260731120000_fms_purchase_qc_inspection.sql. The two Purchase FMS apps
-- are clones with separate table families, so every change lands twice; only the
-- stage machine in section 9 genuinely differs (Import is a pure quantity
-- requisition — no money side).
--
...
-- ---------------------------------------------------------------------------
alter table public.fms_import_categories
  add column if not exists qc_required boolean not null default false;

comment on column public.fms_import_categories.qc_required is
...
update public.fms_import_categories set qc_required = true where upper(name) = 'RAW MATERIAL';
...
-- ---------------------------------------------------------------------------
alter table public.fms_import_grns
  add column if not exists qc_waived_at timestamptz;

comment on column public.fms_import_grns.qc_waived_at is
...
update public.fms_import_grns set qc_waived_at = now() where qc_waived_at is null;
...
-- ---------------------------------------------------------------------------
create table if not exists public.fms_import_qc_inspections (
  ...
  -- Step 9 — Purchase Return Entry in Tally (rejected inspections only).
...
create unique index if not exists fms_import_qc_grn_uniq on public.fms_import_qc_inspections (grn_id);
create index if not exists fms_import_qc_po_idx on public.fms_import_qc_inspections (po_id);
...
create table if not exists public.fms_import_qc_items (
  id            uuid primary key default gen_random_uuid(),
  inspection_id uuid not null references public.fms_import_qc_inspections on delete cascade,
  po_item_id    uuid not null references public.fms_import_po_items on delete cascade,
...
create index if not exists fms_import_qc_items_inspection_idx on public.fms_import_qc_items (inspection_id);

-- RLS — select-all; direct writes admin-only (the RPCs below are definer).
do $$
declare t text;
begin
  foreach t in array array['fms_import_qc_inspections','fms_import_qc_items'] loop
...
-- ---------------------------------------------------------------------------
create or replace function public.fms_import_grn_needs_qc(p_grn_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1
      ...
      join public.fms_import_categories c on c.id = ri.category_id
     where gi.grn_id = p_grn_id
       and c.qc_required
  );
$$;
grant execute on function public.fms_import_grn_needs_qc(uuid) to authenticated;
...
create or replace function public.fms_import_qc_editable(p_inspection_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.fms_import_qc_inspections q
      join public.fms_import_pos po on po.id = q.po_id
     where q.id = p_inspection_id
       and po.current_stage <> 'cancelled'
       and q.return_tally_ref is null
  );
$$;
grant execute on function public.fms_import_qc_editable(uuid) to authenticated;
...
-- ---------------------------------------------------------------------------
-- 6. STEP 9 — PURCHASE RETURN ENTRY IN TALLY.
--
...
-- ---------------------------------------------------------------------------
create or replace function public.fms_import_record_purchase_return(
  p_inspection_id uuid,
  p_tally_ref     text,
...
begin
  if not (public.is_admin(auth.uid()) or public.fms_import_is_step_owner('purchase_return', auth.uid())) then
  ...
  select po_id, result, return_tally_ref into v_po, v_result, v_existing
    from public.fms_import_qc_inspections where id = p_inspection_id for update;
  if v_po is null then raise exception 'QC inspection not found'; end if;
  ...
  perform public.fms_import_refresh_po(v_po);
  select po_no into v_po_no from public.fms_import_pos where id = v_po;
  perform public.fms_import_announce('po', v_po, 'purchase_return_entered',
...
grant execute on function public.fms_import_record_purchase_return(uuid, text, text, text, text) to authenticated;

create or replace function public.fms_import_purchase_return_editable(p_inspection_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.fms_import_qc_inspections q
      join public.fms_import_pos po on po.id = q.po_id
...
$$;
grant execute on function public.fms_import_purchase_return_editable(uuid) to authenticated;

create or replace function public.fms_import_update_purchase_return(
  p_inspection_id uuid,
  p_tally_ref     text,
...
begin
  ...
  select po_id into v_po from public.fms_import_qc_inspections where id = p_inspection_id for update;
  if v_po is null then raise exception 'QC inspection not found'; end if;
  if not public.fms_import_purchase_return_editable(p_inspection_id) then
  ...
  perform public.fms_import_refresh_po(v_po);
  select po_no into v_po_no from public.fms_import_pos where id = v_po;
  perform public.fms_import_announce('po', v_po, 'purchase_return_edited',
    format('Purchase return entry on %s edited', coalesce(v_po_no,'the PO')),
    '{}'::uuid[], jsonb_build_object('po_id', v_po, 'inspection_id', p_inspection_id));
end $$;
grant execute on function public.fms_import_update_purchase_return(uuid, text, text, text, text) to authenticated;

-- ---------------------------------------------------------------------------
-- 7. STEP 10 — GATE REGISTER OUTWARD. Closes the rejection branch.
...
-- ---------------------------------------------------------------------------
create or replace function public.fms_import_record_gate_outward(
  p_inspection_id   uuid,
...
begin
  if not (public.is_admin(auth.uid()) or public.fms_import_is_step_owner('gate_outward', auth.uid())) then
  ...
  perform public.fms_import_refresh_po(v_po);
  select po_no into v_po_no from public.fms_import_pos where id = v_po;
  perform public.fms_import_announce('po', v_po, 'gate_outward_recorded',
    format('Rejected material gated out for %s', coalesce(v_po_no,'the PO')),
    '{}'::uuid[], jsonb_build_object('po_id', v_po, 'inspection_id', p_inspection_id));
end $$;
grant execute on function public.fms_import_record_gate_outward(uuid, text, date, text, text, text) to authenticated;

create or replace function public.fms_import_gate_outward_editable(p_inspection_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.fms_import_qc_inspections q
      join public.fms_import_pos po on po.id = q.po_id
...
$$;
grant execute on function public.fms_import_gate_outward_editable(uuid) to authenticated;

create or replace function public.fms_import_update_gate_outward(
  p_inspection_id    uuid,
  p_gate_register_no text,
  p_out_date         date default null,
  p_remarks          text default null,
...
begin
  ...
  select po_id into v_po from public.fms_import_qc_inspections where id = p_inspection_id for update;
  if v_po is null then raise exception 'QC inspection not found'; end if;
  if not public.fms_import_gate_outward_editable(p_inspection_id) then
    raise exception 'This PO is cancelled — its gate outward can no longer be edited.';
  end if;
  if nullif(trim(p_gate_register_no),'') is null then raise exception 'The gate register number is required'; end if;

  v_date := coalesce(p_out_date, (select gate_out_date from public.fms_import_qc_inspections where id = p_inspection_id));
  if v_date > (now() at time zone 'Asia/Kolkata')::date then
    raise exception 'The outward date cannot be in the future';
  end if;
  ...
  perform public.fms_import_refresh_po(v_po);
  select po_no into v_po_no from public.fms_import_pos where id = v_po;
  perform public.fms_import_announce('po', v_po, 'gate_outward_edited',
    format('Gate register outward on %s edited', coalesce(v_po_no,'the PO')),
    '{}'::uuid[], jsonb_build_object('po_id', v_po, 'inspection_id', p_inspection_id));
end $$;
grant execute on function public.fms_import_update_gate_outward(uuid, text, date, text, text, text) to authenticated;

-- ---------------------------------------------------------------------------
-- 8. LOCK THE TALLY BOOKING ONCE QC HAS CAPTURED IT.
--    `fms_import_tally_editable` gated only on "is the PO open". A PO now stays
...
--    captures it" rule. Body carried forward from 20260719120000 with the extra
--    clause. (`fms_import_grn_editable` needs no change: it is already locked
  ...
  select exists (
    select 1 from public.fms_import_tally_bookings t
     where t.id = p_booking_id
       and public.fms_import_po_open(t.po_id)
       and not exists (select 1 from public.fms_import_qc_inspections q where q.grn_id = t.grn_id)
  );
$$;

-- ---------------------------------------------------------------------------
-- 9. THE STAGE MACHINE. Body carried forward verbatim from 20260727120000 (the
--    quantity-requisition rework) with one change: three new arms —
--    qc_inspection / purchase_return / gate_outward — between the Tally arm and
--    'closed', and the close arm additionally requiring the QC branch to be clear.
--
--    Import already treats only 'cancelled' as absorbing (since 20260724130000),
--    so `closed` is re-derivable here and needed no change: a PO that closed on an
--    earlier receipt walks back into QC when a later booking lands.
-- ---------------------------------------------------------------------------
...
begin
  -- Only cancellation is absorbing. 'closed' is derived and re-derivable.
  if (select current_stage from public.fms_import_pos where id = p_po_id) = 'cancelled' then
    return;
  end if;

  update public.fms_import_po_items pi
     set received_qty = coalesce((select sum(gi.received_qty)
                                    from public.fms_import_grn_items gi
                                   where gi.po_item_id = pi.id), 0)
  ...
  select exists(
    select 1 from public.fms_import_grns gr
     join public.fms_import_tally_bookings t on t.grn_id = gr.id
     where gr.po_id = p_po_id
       and gr.qc_waived_at is null
       and public.fms_import_grn_needs_qc(gr.id)
       and not exists (select 1 from public.fms_import_qc_inspections q where q.grn_id = gr.id)
  ) into v_qc_pending;
  ...
  select exists(select 1 from public.fms_import_followups
                 where po_id = p_po_id and dispatch_status = 'dispatched') into v_dispatched;

  update public.fms_import_pos
     set current_stage = case
           when coalesce(v_all_recv,false) and coalesce(v_tally,false) and not coalesce(v_unbooked_grn,false)
