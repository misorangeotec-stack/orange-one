-- ROLLBACK for 20261129120000_kpi2_team.sql — removes kpi_team and puts kpi_report back exactly as
-- 20261128123000_kpi1_report_trend_to_date.sql left it, then drops the shared rule functions
-- (nothing else calls them once kpi_report no longer does).

drop function if exists public.kpi_team(date, date);
...
drop function if exists public.kpi_trend_weeks(date, date, date);
drop function if exists public.kpi_period(date, date);
drop function if exists public.kpi_is_on_time(text, boolean, date, date);
drop function if exists public.kpi_is_given(text, date, date);
