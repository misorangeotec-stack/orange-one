-- Purchase FMS (procurement) — SINGLE STATE AXIS: current_stage only.
--
...
begin
  ...
  select exists(select 1 from public.fms_purchase_payments
...
update public.fms_purchase_pos set current_stage = 'closed'    where status = 'closed'    and current_stage <> 'closed';
update public.fms_purchase_pos set current_stage = 'cancelled' where status = 'cancelled' and current_stage <> 'cancelled';
