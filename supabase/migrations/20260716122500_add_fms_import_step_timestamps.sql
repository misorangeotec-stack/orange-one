-- ===========================================================================
-- Purchase FMS (import) — AUTHORITATIVE step-completion timestamps.
--
...
-- row in fms_import_activity, which that migration itself calls "best-effort,
...
comment on column public.fms_import_request_items.sourced_at is
  'When sourcing was saved for this line (re-sourcing restarts the clock).';
comment on column public.fms_import_request_items.approved_at is
  'When the line was approved/overridden. Cleared if the approval is resumed.';
comment on column public.fms_import_pos.shared_at is
...
drop function if exists public.fms_import_share_po(uuid, text, text, text, text, text, date);
...
update public.fms_import_request_items ri
   set sourced_at = coalesce(
     (select min(a.created_at) from public.fms_import_activity a
