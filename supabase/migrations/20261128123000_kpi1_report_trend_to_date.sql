-- KPI-1 · The score-by-week trend stops at the current week.
--
-- The trend listed every week up to the end of the period, so a running month showed bars for
-- weeks that had not started yet — and those bars were not empty: a step finished AHEAD of its
-- due date counts as given and done, so a future week showed a score built only from early work
-- (Saloni Rathod's September showed W39, 21–27 Sep, at ~100 on 18-09-2026). A week that has not
-- begun has no score. The user caught it; this ends the series at the as-of date.
--
-- ONE edit to kpi_report as 20261128122000 left it, generated from that file: the weeks series
-- ends at least(p_to, a) instead of p_to. Nothing else changes — the period's own figures, which
-- by the settled rule do include work already finished ahead of its due date, are untouched.
--
-- Rollback: …_kpi1_report_trend_to_date_rollback.sql restores 20261128122000's function verbatim.
...
begin
  ...
  weeks as (
    ...
    select w::date as week_start, w::date as b_from, w::date + 6 as b_to
      -- Never a week that has not started: a future week has no score, only work finished early.
      from generate_series(date_trunc('week', v_tf)::date, least(p_to, a), interval '7 days') w
  )
  select
