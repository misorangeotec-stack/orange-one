-- CC-1 · "Preview as" on the ranking board.
--
-- Admins are never ranked, so an admin opening the Control Center only ever sees
-- "Admins are not ranked" — never the screen an employee gets. This lets an admin ask
-- the board for exactly one person's view: their card, the top five and the employees
-- of the month, cut exactly as for them, and without the admin section.
--
-- The one change to fms_rank_board is the optional p_as, honoured only for an admin.
-- The old one-argument signature is dropped first: kept alongside, a call with only
-- p_month would be ambiguous between the two.
-- Rollback: …_cc1_fms_rank_board_preview_rollback.sql restores the one-argument form.
...
revoke all on function public.fms_rank_board(date, uuid) from public, anon;
grant execute on function public.fms_rank_board(date, uuid) to authenticated;
