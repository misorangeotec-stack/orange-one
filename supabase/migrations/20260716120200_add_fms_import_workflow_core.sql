-- Purchase FMS (import) — WORKFLOW CORE (Phase 3): Stages 1–4.
--
-- The relational spine for Request → Sourcing → tiered Approval → vendor-wise PO:
--   fms_import_requests       — Stage-1 header (buyer company + category)
--   fms_import_request_items  — the line-level state machine (Stages 1–4)
--   fms_import_quotations     — up to 3 per line; one recommended
--   fms_import_pos            — vendor × company PO header
--   fms_import_po_items       — approved lines pulled onto a PO (one active PO/line)
--
-- Plus: fms_import_is_step_owner(text,uuid) authz helper, fms_import_fy_code(date)
...
--
-- Line status machine (fms_import_request_items.status):
--   sourcing → approval → approved_pending_po → po
--                approval → on_hold → approval
--                approval → rejected
--   (any non-po, non-terminal) → cancelled
--
-- Purely ADDITIVE. Reuses set_updated_at / is_admin / fms_import_next_seq.
...
-- Owner check for a workflow step (reads fms_import_step_owners).
create or replace function public.fms_import_is_step_owner(p_step_key text, p_uid uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    select 1 from public.fms_import_step_owners o
...
create or replace function public.fms_import_fy_code(p_d date)
returns text
language sql
immutable
as $$
...
create table if not exists public.fms_import_requests (
  id            uuid primary key default gen_random_uuid(),
  request_no    text not null unique,
  company_id    uuid not null references public.fms_import_companies on delete restrict,
  category_id   uuid not null references public.fms_import_categories on delete restrict,
...
create index if not exists fms_import_requests_company_idx on public.fms_import_requests (company_id);
create index if not exists fms_import_requests_requester_idx on public.fms_import_requests (requester_id);

drop trigger if exists trg_fms_import_requests_updated on public.fms_import_requests;
create trigger trg_fms_import_requests_updated before update on public.fms_import_requests
  for each row execute function public.set_updated_at();

-- ---- request items (the line state machine) ------------------------------
create table if not exists public.fms_import_request_items (
  id             uuid primary key default gen_random_uuid(),
  request_id     uuid not null references public.fms_import_requests on delete cascade,
  item_id        uuid not null references public.fms_import_items on delete restrict,
  quantity       numeric(14,3) not null check (quantity > 0),
  unit           text not null default '',
  line_remark    text,
  sourcing_reason text,
  final_vendor_id uuid references public.fms_import_vendors on delete set null,
...
create index if not exists fms_import_request_items_req_idx on public.fms_import_request_items (request_id);
create index if not exists fms_import_request_items_status_idx on public.fms_import_request_items (status);
create index if not exists fms_import_request_items_vendor_idx on public.fms_import_request_items (final_vendor_id);

drop trigger if exists trg_fms_import_request_items_updated on public.fms_import_request_items;
create trigger trg_fms_import_request_items_updated before update on public.fms_import_request_items
  for each row execute function public.set_updated_at();

-- ---- quotations -----------------------------------------------------------
create table if not exists public.fms_import_quotations (
  id              uuid primary key default gen_random_uuid(),
  request_item_id uuid not null references public.fms_import_request_items on delete cascade,
  vendor_id       uuid not null references public.fms_import_vendors on delete restrict,
...
create index if not exists fms_import_quotations_line_idx on public.fms_import_quotations (request_item_id);

-- ---- POs (vendor × company header) ---------------------------------------
create table if not exists public.fms_import_pos (
  id            uuid primary key default gen_random_uuid(),
...
create index if not exists fms_import_pos_vendor_idx on public.fms_import_pos (vendor_id);
create index if not exists fms_import_pos_company_idx on public.fms_import_pos (company_id);

drop trigger if exists trg_fms_import_pos_updated on public.fms_import_pos;
create trigger trg_fms_import_pos_updated before update on public.fms_import_pos
  for each row execute function public.set_updated_at();

-- ---- PO items (approved lines pulled onto a PO) --------------------------
create table if not exists public.fms_import_po_items (
  id              uuid primary key default gen_random_uuid(),
  po_id           uuid not null references public.fms_import_pos on delete cascade,
  request_item_id uuid not null unique references public.fms_import_request_items on delete restrict,
...
create index if not exists fms_import_po_items_po_idx on public.fms_import_po_items (po_id);
...
begin
  foreach t in array array[
    'fms_import_requests','fms_import_request_items','fms_import_quotations',
    'fms_import_pos','fms_import_po_items'
  ] loop
...
begin
  ...
  insert into public.fms_import_requests (request_no, company_id, category_id, requester_id, note)
  ...
  for v_elem in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.fms_import_request_items (request_id, item_id, quantity, unit, line_remark)
    values (
      v_request_id,
      (v_elem->>'item_id')::uuid,
...
grant execute on function public.fms_import_submit_request(uuid, uuid, text, jsonb) to authenticated;
...
begin
  ...
  update public.fms_import_pos set total_value = v_total where id = v_po_id;
  return v_po_id;
end $$;
...
begin
  select status into v_status from public.fms_import_request_items where id = p_request_item_id for update;
  if v_status is null then raise exception 'Line not found'; end if;
  if not (public.is_admin(auth.uid())
          or public.fms_import_is_step_owner('po', auth.uid())
          or public.fms_import_is_step_owner('sourcing', auth.uid())) then
