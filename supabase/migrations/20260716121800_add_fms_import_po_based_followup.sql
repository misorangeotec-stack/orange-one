-- ===========================================================================
-- Purchase FMS (import) — follow-up is recorded against the PO, and the
-- advance step is driven by the PO's payment terms (not the PI's).
--
--   • fms_import_followups.pi_id becomes nullable (data-safe) and gains an
...
--   • refresh_po now reads advance-need from fms_import_pos.payment_terms and
...
alter table public.fms_import_followups alter column pi_id drop not null;
alter table public.fms_import_followups add column if not exists pi_remarks text;
create index if not exists fms_import_followups_po_idx on public.fms_import_followups (po_id);
...
begin
  ...
  if not exists (select 1 from public.fms_import_pos where id = p_po_id) then raise exception 'PO not found'; end if;
...
grant execute on function public.fms_import_record_followup(uuid, text, date, text, text, date, text, text) to authenticated;
