-- Purchase FMS (import) — Book-in-Tally captures the TALLY INVOICE.
--
...
alter table public.fms_import_tally_bookings
  add column if not exists document_path text,
  add column if not exists document_name text,
  add column if not exists remarks       text;

-- Replace the RPC with the wider signature (old signature dropped first).
drop function if exists public.fms_import_book_tally(uuid, text, uuid);
