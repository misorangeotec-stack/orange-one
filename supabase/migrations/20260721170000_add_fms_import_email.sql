-- Email Alerts — Import FMS rollout (step 1 of the app: enqueue).
--
-- WHY
--   Import already fans every workflow transition through ONE RPC,
--   public.fms_import_announce, which computes the exact next-step recipient(s)
...
--   Import bell notification goes, inheriting the RPC's self-skip + de-dup.
--
--   Rich per-step content is authored client-side (where Import's selectors +
--   ₹/foreign formatters + itemLabel live) and passed in p_meta; it is carried
--   verbatim into email_outbox.payload and rendered by the send-email function's
--   Import template. kind = 'import_' || p_type.
--
-- GATE: only fires when email_module_enabled('import') is true; seeded OFF below,
-- so nothing emails until an admin flips the Import → Setup → Notifications switch.
--
-- Additive + reversible: re-apply 20260716120400_add_fms_import_activity_notifications.sql
-- to restore the un-enqueuing body; delete the seeded row to remove the gate.

-- Seed the per-module gate row (OFF). email_module_enabled('import') is false
...
begin
  ...
  if p_user_ids is not null then
    foreach u in array p_user_ids loop
      ...
      -- (new) email the same recipient, only when Import email is enabled.
      if v_email_on then
        begin
          v_email := coalesce(
