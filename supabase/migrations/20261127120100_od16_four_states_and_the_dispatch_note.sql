-- ===========================================================================
-- OD-16 PHASE 2 — FOUR WORDS FOR THE CUSTOMER, AND ONE NOTE THEY CAN READ.
--
--   1. THE CUSTOMER SEES FOUR STATES, NOT EIGHT: Request raised → Accepted →
--      Out for delivery → Delivered, plus Cancelled. Our flow has eleven
--      statuses and six steps; the customer was being shown a collapse of them
--      that still leaked the shape of our process ("Being prepared", "Paused",
--      "Partly dispatched"). They asked for the four words they actually use.
--
--   2. AFTER IT GOES OUT, THEY READ ONE NOTE ABOUT IT — and nothing else. A new
--      field, written at the gate outward entry, is the ONLY remark on the order
--      that ever reaches them.
--
-- ⚠ CANCELLING IS UNCHANGED AND STILL ALLOWED. An earlier draft of this
--   migration made `fms_dispatch_cancel_customer_order` refuse outright; the
--   instruction is narrower than that and the function is left alone. A customer
--   may cancel WHILE THE ORDER READS "Request raised" — which is exactly
--   `fms_dispatch_customer_window_open`, the function that already gated it. Once
--   we accept, the window shuts and both cancel and change stop. No new rule was
--   needed; the state names just had to line up with the one already there.
--
-- ⚠ WHY A NEW COLUMN AND NOT `go_remarks`. `go_remarks` is ours: it is written
--   for the next person in the chain, it names steps and people, and 400-odd rows
--   of it already exist written under the assurance that no customer would read
--   them. Surfacing it would publish that backlog retroactively, to the customers
--   it discusses. A field that is customer-facing from its first row cannot do
--   that, and the person typing it can see which box they are in.
-- ===========================================================================

begin;

-- ===========================================================================
-- 1. THE ONE REMARK THE CUSTOMER READS
-- ===========================================================================
-- On the header (the live round) and on the archive, because every other step
-- field is on both and a round that closes has to carry its note with it.
alter table public.fms_dispatch_orders
  add column if not exists go_customer_remark text;

alter table public.fms_dispatch_rounds
  add column if not exists go_customer_remark text;

comment on column public.fms_dispatch_orders.go_customer_remark is
  'OD-16. Written at the gate outward entry and shown to the CUSTOMER on the Order Desk. The '
  'only remark on this order that ever leaves the company — go_remarks stays internal.';

comment on column public.fms_dispatch_rounds.go_customer_remark is
  'OD-16. The customer-facing outward note for this round, archived from the header.';

-- ===========================================================================
-- 2. WRITING IT — the gate outward entry
-- ===========================================================================
-- ⚠ BOTH FUNCTIONS RESTATED IN FULL, bodies otherwise identical to
--   20260928120000_fms_dispatch_gate_pass_location_series. The only change in
--   each is the one new assignment.
...
begin
  ...
  update public.fms_dispatch_orders set
    ...
    go_customer_remark = nullif(trim(p->>'go_customer_remark'), ''),
...
-- ===========================================================================
-- 3. ARCHIVING IT — fms_dispatch_archive_round
-- ===========================================================================
-- ⚠ RESTATED FROM 20261122120000_od15_one_shipment_several_lots VERBATIM except
--   for `go_customer_remark` in three places: the insert column list, the select
--   beside `go_remarks`, and the loop-back wipe. It travels with the go_ block
--   in all three, because it describes THIS round's consignment and a new round
--   goes out under a new note.
...
begin
  insert into public.fms_dispatch_rounds (
    ...
    gp_no,
    go_actual_date, go_outward_no, go_remarks, go_customer_remark, go_at, go_by,
  ...
  select
    ...
    o.gp_no,
    o.go_actual_date, o.go_outward_no, o.go_remarks, o.go_customer_remark, o.go_at, o.go_by,
  ...
  update public.fms_dispatch_orders set
    ...
    gp_no = null,
    go_actual_date = null, go_outward_no = null, go_remarks = null,
    go_customer_remark = null, go_at = null, go_by = null,
...
-- ===========================================================================
-- 5. WHAT THE ORDER DESK READS BACK
-- ===========================================================================
-- Gains `form_name` (OD-16 phase 1 — the customer reads the form, not our
-- company) and `dispatch_notes` (this migration).
--
-- ⚠ `dispatch_notes` IS PAST ROUNDS *AND* THE LIVE HEADER, and missing either is
--   a silent half-answer. A part-dispatched order has its first consignment in
--   `fms_dispatch_rounds` and its current one still on the header; reading only
--   the archive hides the note on the consignment that just left, and reading
--   only the header hides every note before it.
--
-- ⚠ AND IT IS GATED ON `go_at`, NOT ON THE ORDER'S STATUS. The instruction is
--   "after it is dispatched or gets outward" — which is exactly the moment the
--   gate outward entry is stamped. A status test would also admit an order the
--   gate has not touched, and on a looped-back round the header note is wiped
--   anyway, so there is nothing to leak.
...
returns table(id uuid, order_no text, order_date date, order_remarks text,
              ...
              company_id uuid, company_label text, form_name text,
              dispatch_notes jsonb, lines jsonb)
...
comment on function public.fms_dispatch_my_orders() is
  'OD-16. The signed-in customer''s orders. form_name is what they place orders on (NULL falls '
  'back to company_label); dispatch_notes carries the gate-outward notes written FOR them, and '
  'is the only remark channel that reaches a customer.';
