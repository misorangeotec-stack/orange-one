-- Purchase FMS (import) — GRN PHOTO capture.
--
...
-- fms_import_grns, and record_grn extended to persist them. Photos live in the
-- existing `fms-import-docs` storage bucket (same as PI/PO documents).

alter table public.fms_import_grns add column if not exists photo_path text;
alter table public.fms_import_grns add column if not exists photo_name text;
...
drop function if exists public.fms_import_record_grn(uuid, jsonb, uuid, text, text, text);
...
begin
  ...
  insert into public.fms_import_grns (po_id, pi_id, gate_register_no, condition, note, received_by, photo_path, photo_name)
...
grant execute on function public.fms_import_record_grn(uuid, jsonb, uuid, text, text, text, text, text) to authenticated;
