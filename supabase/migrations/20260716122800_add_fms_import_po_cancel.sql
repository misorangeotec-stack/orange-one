-- ===========================================================================
-- Purchase FMS (import) — approver-only PO CANCELLATION (vendor-requested).
--
...
-- request lines (approval routes PER-LINE; fms_import_pos.total_value is only a
...
--    Mirrors the fms_import_master_requests request→resolve shape.
-- ---------------------------------------------------------------------------
create table if not exists public.fms_import_po_cancel_requests (
...
create index if not exists fms_import_po_cancel_requests_po_idx
  on public.fms_import_po_cancel_requests (po_id);
-- At most one OPEN cancellation request per PO. A re-request after a decline is
-- allowed (the index only covers 'pending' rows).
create unique index if not exists fms_import_po_cancel_requests_one_pending
  on public.fms_import_po_cancel_requests (po_id) where status = 'pending';

drop trigger if exists trg_fms_import_po_cancel_requests_updated on public.fms_import_po_cancel_requests;
create trigger trg_fms_import_po_cancel_requests_updated before update on public.fms_import_po_cancel_requests
...
alter table public.fms_import_po_cancel_requests enable row level security;
drop policy if exists fms_import_po_cancel_requests_select on public.fms_import_po_cancel_requests;
create policy fms_import_po_cancel_requests_select on public.fms_import_po_cancel_requests
  for select to authenticated using (true);
drop policy if exists fms_import_po_cancel_requests_write_admin on public.fms_import_po_cancel_requests;
create policy fms_import_po_cancel_requests_write_admin on public.fms_import_po_cancel_requests
  for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

-- ---------------------------------------------------------------------------
-- 3a. Stage 5–10 owners (or admin) may LOG a vendor cancellation request.
-- ---------------------------------------------------------------------------
create or replace function public.fms_import_request_po_cancel(
...
begin
  if not (public.is_admin(auth.uid())
          or public.fms_import_is_step_owner('share_po', auth.uid())
          or public.fms_import_is_step_owner('collect_pi', auth.uid())
          or public.fms_import_is_step_owner('advance_payment', auth.uid())
          or public.fms_import_is_step_owner('follow_up', auth.uid())
          or public.fms_import_is_step_owner('inward', auth.uid())
          or public.fms_import_is_step_owner('tally', auth.uid())) then
    raise exception 'Not authorized to request a PO cancellation';
  end if;
  if coalesce(btrim(p_reason),'') = '' then raise exception 'A reason is required'; end if;

  select current_stage into v_stage from public.fms_import_pos where id = p_po_id for update;
  ...
  if exists (select 1 from public.fms_import_grns where po_id = p_po_id) then
    raise exception 'Goods already received — this PO can no longer be cancelled';
  end if;
  if exists (select 1 from public.fms_import_tally_bookings where po_id = p_po_id) then
    raise exception 'Already booked in Tally — this PO can no longer be cancelled';
  end if;

  insert into public.fms_import_po_cancel_requests (po_id, reason, vendor_ref, requested_by)
  values (p_po_id, btrim(p_reason), nullif(btrim(p_vendor_ref),''), auth.uid())
  returning id into v_id;
  return v_id;
end $$;
grant execute on function public.fms_import_request_po_cancel(uuid, text, text) to authenticated;
...
-- ---------------------------------------------------------------------------
create or replace function public.fms_import_cancel_po(
...
begin
  ...
  if not (public.is_admin(auth.uid())
          or exists (
            select 1
              from public.fms_import_po_items poi
  ...
  update public.fms_import_request_items ri
     set status = 'cancelled',
         cancel_reason = btrim(p_reason)
   where ri.id in (
     select poi.request_item_id from public.fms_import_po_items poi where poi.po_id = p_po_id
   );

  if p_request_id is not null then
    update public.fms_import_po_cancel_requests
       set status = 'approved', reviewed_by = auth.uid()
     where id = p_request_id and po_id = p_po_id and status = 'pending';
  end if;
end $$;
grant execute on function public.fms_import_cancel_po(uuid, text, uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- 3c. The approver (or admin) may DECLINE a cancellation request. PO stays open.
-- ---------------------------------------------------------------------------
create or replace function public.fms_import_decline_po_cancel(
...
begin
  select po_id into v_po_id from public.fms_import_po_cancel_requests
...
grant execute on function public.fms_import_decline_po_cancel(uuid, text) to authenticated;
...
begin
  ...
  if (select current_stage from public.fms_import_pos where id = p_po_id) in ('cancelled','closed') then
    return;
  end if;
