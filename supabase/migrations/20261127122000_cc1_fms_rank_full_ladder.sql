-- CC-1 · The full ladder for everyone, and a provisional place under 10 steps.
--
-- Decided by the user on 18-09-2026, after seeing the panel:
--   · EVERY viewer sees the whole ladder — each ranked person's rank, name, score and
--     step count — to see where they stand against every colleague. (Until now names
--     left the database only for the top five, the viewer and the employees of the
--     month.) A person's split by process and their step-by-step detail stay private.
--   · Someone under the 10-step minimum sees where they WOULD sit today, marked
--     provisional. The official ladder, the podium and the employees of the month still
--     need 10 steps, so one on-time step at 100% cannot top the company.
--
-- Same signature, so CREATE OR REPLACE. Rollback: the _rollback.sql beside this restores
-- the board as 20261127121000_cc1_fms_rank_board_preview.sql left it.
...
begin
  ...
  v_out := jsonb_build_object(
    ...
    -- THE FULL LADDER, NAMED — opened to every viewer by the user on 18-09-2026 so each
    -- person can see where they stand against every colleague. Rank, name, score and
    -- steps only: a per-process split and the step-by-step detail stay the owner's.
    'ladder', coalesce((select jsonb_agg(jsonb_build_object(
                  ...
                  order by s.rank, s.given desc, p.name)
                 from public.fms_rank_scores s join public.profiles p on p.id = s.user_id
                where s.month = v_month and s.ranked), '[]'::jsonb),
    ...
    'me', case when v_me.user_id is null then null else jsonb_build_object(
            ...
            'gap', v_gap,
            -- Under the 10-step minimum: where they WOULD sit if the ladder closed today —
            -- shown to them as provisional. They stay off the official ladder, the podium
            -- and the employees of the month until they reach 10.
            'provisional_rank', case when v_me.not_ranked = 'under_minimum' then 1 +
                (select count(*) from public.fms_rank_scores o
                  where o.month = v_month and o.ranked and o.score > v_me.score) end) end,
