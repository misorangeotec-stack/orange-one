-- Purchase FMS (procurement) — ACTIVITY + NOTIFICATIONS (Phase 5).
--
...
--
-- Tables:
--   fms_purchase_activity       — immutable audit trail across every entity
--   fms_purchase_notifications  — per-user bell feed (own-row read; mark-read)
--
-- Column:
--   fms_purchase_request_items.assigned_approver_id — optional per-line approver
--     override, set by a coordinator/admin Reassign; honored by decide_approval.
--
-- RPCs (all SECURITY DEFINER):
--   fms_purchase_announce       — one call writes an activity row (actor = caller)
...
--   fms_purchase_reassign_line  — coordinator/admin sets a line's override approver,
--                                  then announces it to that approver.
--   fms_purchase_decide_approval (re-created) — authz now also accepts the
--                                  assigned_approver_id, so a reassigned approver
--                                  can act. Body otherwise unchanged.
--
-- Purely ADDITIVE. Reuses is_admin / fms_purchase_is_step_owner / the approval
-- matrix. Reversal: drop the 3 RPCs (restore prior decide_approval), drop the
-- column, then tables fms_purchase_notifications, fms_purchase_activity.

-- ===========================================================================
-- TABLES
-- ===========================================================================
create table if not exists public.fms_purchase_activity (
...
create index if not exists fms_purchase_activity_entity_idx on public.fms_purchase_activity (entity_type, entity_id);
create index if not exists fms_purchase_activity_created_idx on public.fms_purchase_activity (created_at);

create table if not exists public.fms_purchase_notifications (
  id          uuid primary key default gen_random_uuid(),
...
create index if not exists fms_purchase_notifications_user_idx on public.fms_purchase_notifications (user_id, read_at);
create index if not exists fms_purchase_notifications_created_idx on public.fms_purchase_notifications (created_at);
...
-- ===========================================================================
alter table public.fms_purchase_activity enable row level security;
drop policy if exists fms_purchase_activity_select on public.fms_purchase_activity;
create policy fms_purchase_activity_select on public.fms_purchase_activity
  for select to authenticated using (true);
drop policy if exists fms_purchase_activity_write_admin on public.fms_purchase_activity;
create policy fms_purchase_activity_write_admin on public.fms_purchase_activity
  for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));

alter table public.fms_purchase_notifications enable row level security;
drop policy if exists fms_purchase_notifications_select_own on public.fms_purchase_notifications;
create policy fms_purchase_notifications_select_own on public.fms_purchase_notifications
  for select to authenticated using (user_id = auth.uid());
drop policy if exists fms_purchase_notifications_update_own on public.fms_purchase_notifications;
create policy fms_purchase_notifications_update_own on public.fms_purchase_notifications
  for update to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
drop policy if exists fms_purchase_notifications_write_admin on public.fms_purchase_notifications;
create policy fms_purchase_notifications_write_admin on public.fms_purchase_notifications
  for all to authenticated using (public.is_admin(auth.uid())) with check (public.is_admin(auth.uid()));
...
create or replace function public.fms_purchase_is_coordinator(p_uid uuid)
returns boolean
language sql
stable
security definer
...
$$;
grant execute on function public.fms_purchase_is_coordinator(uuid) to authenticated;
...
create or replace function public.fms_purchase_reassign_line(
...
begin
  if not public.fms_purchase_is_coordinator(auth.uid()) then
...
grant execute on function public.fms_purchase_reassign_line(uuid, uuid, text) to authenticated;
