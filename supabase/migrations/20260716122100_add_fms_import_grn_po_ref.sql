-- ===========================================================================
-- Purchase FMS (import) — GRN is referenced by the PO, not the PI.
--
...
alter table public.fms_import_grns add column if not exists po_ref text;
...
update public.fms_import_grns g
   set po_ref = coalesce(p.tally_po_no, p.po_no)
  from public.fms_import_pos p
 where p.id = g.po_id
   and g.po_ref is null;

drop function if exists public.fms_import_record_grn(uuid, jsonb, uuid, text, text, text, text, text, text);
...
begin
  ...
  insert into public.fms_import_grns (po_id, pi_id, po_ref, pi_ref, gate_register_no, condition, note, received_by, photo_path, photo_name)
  values (p_po_id, p_pi_id,
          coalesce(nullif(p_po_ref,''), (select coalesce(tally_po_no, po_no) from public.fms_import_pos where id = p_po_id)),
...
grant execute on function public.fms_import_record_grn(uuid, jsonb, uuid, text, text, text, text, text, text, text) to authenticated;
