// GENERATED by supabase/ranking/build.mjs kpi — do not edit by hand.
...
// frontend/src/apps/kra-kpi/facts/types.ts
var emptyStats = (source) => ({
  source,
  facts: 0,
  closed: 0,
  open: 0,
...
// frontend/src/apps/kra-kpi/facts/fmsFacts.ts
function fmsFacts({ module, moduleName, closed, open }) {
  ...
  for (const c of closed) {
    ...
    const why = c.drop ?? (!c.dueIso ? "untimed" : !c.actorId ? "no_actor" : !doneDate ? "no_time" : void 0);
    if (why) {
...
// frontend/src/apps/kra-kpi/facts/score.ts
var round = (x, dp) => {
  const f = 10 ** dp;
  return Math.round(x * f) / f;
};
var pctNotDone = (c) => c.given > 0 ? round((c.done - c.given) / c.given * 100, 2) : null;
var pctNotOnTime = (c) => c.done > 0 ? round((c.onTime - c.done) / c.done * 100, 2) : null;
var scoreOf = (c) => c.given > 0 ? round((c.onTime + (c.done - c.onTime) / 2) / c.given * 100, 1) : null;
function assertSheetArithmetic() {
...
// frontend/src/apps/task-management/lib/taskCounts.ts
var countsTowardMetrics = (t) => !t.notApplicable && !t.isPersonal && !t.isPeerAssignment;
var isRecurringTask = (t) => t.fromRecurring || t.recurringTaskId !== null;

// frontend/src/apps/kra-kpi/facts/taskFacts.ts
...
function collidingTemplates(templates) {
...
function taskFacts({ tasks, templates, bulkClosed }) {
  ...
  for (const t of tasks) {
    ...
    if (isRecurringTask(t)) {
      const tpl = t.recurringTaskId ? templateById.get(t.recurringTaskId) : void 0;
      if (tpl) {
        rowKey = tpl.id;
...
async function fetchTaskScoringData() {
...
// frontend/src/apps/kra-kpi/facts/taskSource.ts
var BULK_CLOSE_TABLES = ["task_bulk_close_20260630", "task_bulk_close_af_20260909"];
var PAGE = 1e3;
var db = supabase;
async function taskIdsOf(table) {
  const out = [];
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await db.from(table).select("task_id").order("task_id", { ascending: true }).range(from, from + PAGE - 1);
...
async function loadTaskFactsInput() {
...
function createStepSlaModel(steps, overrides = {}) {
  const DEFAULT_STEP_SLA12 = steps.reduce((acc, step, i) => {
    acc[step.key] = {
  ...
  return { DEFAULT_STEP_SLA: DEFAULT_STEP_SLA12, anchorOptions: anchorOptions12, resolveStepSla: resolveStepSla12 };
}
...
var db2 = supabase;
var PAGE2 = 1e3;
async function fetchAll2(table, orderBy = "created_at") {
  const out = [];
  for (let from = 0; ; from += PAGE2) {
    const { data, error } = await db2.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE2 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchAssetData() {
  const [
    ...
    fetchAll2("fms_asset_step_owners"),
    fetchAll2("fms_asset_config", "key"),
    fetchAll2("designations", "name"),
    fetchAll2("departments", "name"),
    fetchAll2("fms_asset_schedule_types", "sort_order"),
    fetchAll2("fms_asset_categories", "sort_order"),
    fetchAll2("fms_asset_locations", "sort_order"),
    fetchAll2("fms_asset_vendors", "sort_order"),
    fetchAll2("fms_asset_makes", "sort_order"),
    fetchAll2("fms_asset_companies", "sort_order"),
    fetchAll2("fms_asset_conditions", "sort_order"),
    fetchAll2("fms_asset_usage_units", "sort_order"),
    fetchAll2("fms_asset_cost_heads", "sort_order"),
    fetchAll2("fms_asset_master_managers"),
    fetchAll2("fms_asset_master_requests"),
    fetchAll2("fms_asset_assets"),
    fetchAll2("fms_asset_schedules"),
    fetchAll2("fms_asset_readings"),
    fetchAll2("fms_asset_jobs"),
    fetchAll2("fms_asset_reminder_log"),
    fetchAll2("fms_asset_activity"),
    fetchAll2("fms_asset_notifications")
  ]);
...
function heldStep(j) {
  const def = j.currentStep ? stepByKey(j.currentStep) : void 0;
  if (!def || def.noQueue) return null;
  return def.key;
}
function buildHeldEntries(snap) {
  const out = [];
...
function assetWorkItems(data, uid, isAdmin) {
  ...
  return entries.filter(({ e }) => isAdmin || isMineByStepOwners(e.stepKey, uid, owners) || isMyAsset(e.stepKey, e.jobId)).map(({ e, held }) => {
...
var heldDrop = (item) => item.isHeld ? "held" : void 0;
...
var CHUNK = 80;
async function fetchAll3(table, orderBy = "created_at", narrow) {
  const out = [];
  for (let from = 0; ; from += PAGE3) {
    let q = supabase.from(table).select("*");
    if (narrow) q = narrow(q);
    const { data, error } = await q.order(orderBy, { ascending: true }).range(from, from + PAGE3 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchCandidatesInScope(liveRequisitionIds) {
  ...
  const batches = await Promise.all([
    fetchAll3("fms_hr_candidates", "created_at", (q) => q.gte("uploaded_at", windowStart)),
    // A
    ...chunks.map((ids) => fetchAll3("fms_hr_candidates", "created_at", (q) => q.in("requisition_id", ids))),
    // B
    ...
    fetchAll3("fms_hr_candidates", "created_at", (q) => q.in("stage", ["finalized", "hired"]))
  ]);
...
async function fetchHrData() {
  const requisitions = await fetchAll3("fms_hr_requisitions");
  ...
  const [
    ...
    fetchAll3("fms_hr_step_owners"),
    fetchAll3("fms_hr_step_assignees", "requisition_id"),
    fetchAll3("fms_hr_config", "key"),
    fetchAll3("designations"),
    fetchAll3("fms_hr_job_platforms"),
    fetchAll3("fms_hr_job_types"),
    fetchAll3("fms_hr_locations"),
    fetchAll3("fms_hr_disqualification_reasons"),
    fetchAll3("fms_hr_onboarding_items"),
    ...
    fetchAll3("fms_hr_job_titles", "sort_order"),
    fetchAll3("fms_hr_skills", "sort_order"),
    fetchAll3("fms_hr_qualifications", "sort_order"),
    fetchAll3("fms_hr_requisition_platforms", "requisition_id"),
    ...
    fetchAll3("fms_hr_interviews"),
    fetchAll3("fms_hr_onboardings"),
    ...
    fetchAll3("fms_hr_notifications"),
    fetchAll3("fms_hr_master_managers"),
    fetchAll3("fms_hr_master_requests"),
    // NR-3 Part B. Keyed on department_id, so ordered by it - the table has no
    // created_at for fetchAll's default to sort on.
    fetchAll3("fms_hr_department_hods", "department_id")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
...
function candidateDueFor(snap, c, step, round2, which, reqById, ivByCandidate) {
  const sla = snap.stepSla[step];
  if (!sla) return null;
  if (round2 !== null) {
    const list = ivByCandidate?.get(c.id) ?? snap.interviews.filter((iv) => iv.candidateId === c.id);
    const booked = list.find((iv) => iv.round === round2 && (which === "held" ? !!iv.heldAt : !iv.heldAt));
...
function buildHeldEntries2(snap) {
  const out = [];
  for (const r of snap.requisitions) {
    if (r.status !== "on_hold") continue;
    const def = stepByKey2(r.currentStep);
...
var interviewResultLockReason = (c, round2) => {
  const want = round2 === 0 ? "telephonic" : `interview_${round2}`;
...
function hrCompletedEntries(ix, stepKey) {
  ...
  switch (stepKey) {
    ...
    case "interview_2":
    case "interview_3": {
      const round2 = stepKey === "telephonic_screening" ? 0 : Number(stepKey.slice(-1));
      const out = [];
      for (const c of ix.candidates) {
        const iv = (ix.ivsByCan.get(c.id) ?? []).find((v) => v.round === round2 && v.heldAt);
        if (!iv) continue;
        out.push(
          stageEntryOf(
            stepKey,
            ...
            iv.resultRecordedBy,
            iv.heldAt,
            interviewResultLockReason(c, round2),
            false
          )
        );
      }
...
function hrWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries2(snap).map((e) => ({ e, held: false })),
    ...buildHeldEntries2(snap).map((e) => ({ e, held: true }))
  ];
  return entries.filter(({ e }) => isAdmin || isMine(e.stepKey, e.requisitionId, e.entityId)).map(({ e, held }) => ({
...
var hrRecruitmentScorer = {
  ...
  openFor(data, uid) {
    ...
    return parseItems(hrWorkItems(data, uid, false)).map(({ entityId, stepKey, item }) => ({
      ...
      roundNo: 0,
      dueIso: item.dueIso,
      drop: heldDrop(item) ?? (EXCLUDED.has(stepKey) ? "excluded_step" : isTestReq(reqOf.get(entityId) ?? null) ? "test_record" : void 0)
    }));
  }
};
...
async function fetchAllOptional(table, orderBy = "created_at") {
  try {
    return await fetchAll4(table, orderBy);
  } catch (e) {
    const msg = e.message ?? "";
...
async function fetchImportData() {
  const [
    ...
    fetchAll4("fms_import_companies"),
    fetchAll4("fms_import_categories"),
    fetchAll4("fms_import_item_groups"),
    fetchAll4("fms_import_items"),
    fetchAll4("fms_import_vendors"),
    fetchAll4("fms_import_vendor_item_prices"),
    fetchAll4("fms_import_master_managers"),
    fetchAll4("fms_import_master_requests"),
    fetchAll4("fms_import_po_cancel_requests"),
    fetchAll4("designations"),
    fetchAll4("fms_import_step_owners"),
    fetchAll4("fms_import_approval_matrix"),
    fetchAll4("fms_import_config", "key"),
    fetchAll4("fms_import_requests"),
    fetchAll4("fms_import_request_items"),
    fetchAll4("fms_import_quotations"),
    fetchAll4("fms_import_pos"),
    fetchAll4("fms_import_po_items"),
    fetchAll4("fms_import_pis"),
    fetchAll4("fms_import_pi_items"),
    fetchAll4("fms_import_grns"),
    fetchAll4("fms_import_grn_items"),
    fetchAll4("fms_import_tally_bookings"),
    fetchAll4("fms_import_qc_inspections"),
    fetchAll4("fms_import_qc_items"),
    fetchAll4("fms_import_payments"),
    fetchAll4("fms_import_followups"),
    fetchAll4("fms_import_activity"),
    fetchAll4("fms_import_notifications"),
...
function importWorkItems(data, uid, isAdmin) {
  ...
  const heldCount = (stepKey, entityId) => {
...
// frontend/src/apps/learning-development/lib/sla.ts
var OVERRIDES2 = {
...
// frontend/src/apps/learning-development/data/ldFetch.ts
var PAGE5 = 1e3;
...
var mapRequest2 = (r) => ({
  ...
  proposedCost: num4(r.proposed_cost),
  ...
  approvedBudget: num4(r.approved_budget),
...
async function fetchLdData() {
  const [
    ...
    fetchAll5("fms_ld_step_owners"),
    fetchAll5("fms_ld_step_assignees", "assigned_at"),
    fetchAll5("fms_ld_config", "key"),
    fetchAll5("fms_ld_session_types", "sort_order"),
    fetchAll5("fms_ld_competencies", "sort_order"),
    fetchAll5("fms_ld_need_sources", "sort_order"),
    fetchAll5("fms_ld_venues", "sort_order"),
    fetchAll5("fms_ld_trainers", "sort_order"),
    fetchAll5("fms_ld_delay_reasons", "sort_order"),
    fetchAll5("fms_ld_followup_actions", "sort_order"),
    fetchAll5("fms_ld_master_managers"),
    fetchAll5("fms_ld_master_requests"),
    fetchAll5("fms_ld_requests"),
    fetchAll5("fms_ld_sessions"),
    fetchAll5("fms_ld_activity"),
    fetchAll5("fms_ld_notifications"),
    fetchAll5("fms_ld_nominations", "nominated_at"),
    fetchAll5("fms_ld_materials", "uploaded_at"),
    fetchAll5("fms_ld_attendance", "marked_at"),
    fetchAll5("fms_ld_assignments", "issued_at"),
    // ⚠ NOT the default "created_at" — this table has none. See the note on fetchAll.
    fetchAll5("fms_ld_assignment_submissions", "updated_at"),
    fetchAll5("fms_ld_feedback", "submitted_at"),
    fetchAll5("fms_ld_effectiveness", "due_on"),
    fetchAll5("fms_ld_plans"),
    fetchAll5("fms_ld_plan_lines", "planned_month"),
    fetchAll5("fms_ld_mandatory_programs", "sort_order")
  ]);
  const cfg = {};
  ...
  return {
    ...
    feedback: feedback.map((r) => ({
      ...
      trainerRating: r.trainer_rating,
      relevanceRating: r.relevance_rating,
    ...
    effectiveness: effectiveness.map((r) => ({
      ...
      rating: r.rating,
      outcome: r.outcome,
      applicationObserved: r.application_observed,
      evidence: r.evidence,
      improvementArea: r.improvement_area,
      followupRequired: r.followup_required ?? false,
      followupActionId: r.followup_action_id,
      submittedAt: r.submitted_at
    })),
    plans: plans.map((r) => ({
      id: r.id,
    ...
    planLines: planLines.map((r) => ({
      ...
      plannedHours: num4(r.planned_hours),
      estimatedCost: num4(r.estimated_cost),
      note: r.note,
      sortOrder: r.sort_order ?? 0
    })),
    ...
    stepSla: resolveStepSla4(cfg.step_sla ?? null),
    coordinatorIds: cfg.process_coordinators?.user_ids ?? [],
    approvalRule: {
      mgmt: rule.mgmt ?? "never",
...
// frontend/src/apps/learning-development/lib/queues.ts
function stepOf(r) {
  switch (r.status) {
    case "draft":
      return null;
...
function stepCompletedIso(r, step) {
  switch (step) {
    case "need_raised":
      return r.submittedAt;
    case "need_resubmit":
...
function dueIsoFor(r, sla) {
  const step = stepOf(r);
  if (!step) return null;
  const rule = sla[step];
  if (!rule) return null;
  const anchorIso = stepCompletedIso(r, rule.anchor) ?? r.submittedAt;
  return dueIsoFrom(anchorIso, rule);
}
var isOpen = (r) => !["closed", "rejected", "cancelled"].includes(r.status);
function buildQueueEntries4(requests, sla) {
  const out = [];
...
// frontend/src/apps/learning-development/lib/work.ts
var dayOf = (iso) => iso ? iso.slice(0, 10) : null;
var dueFrom = (sla, step, anchorIso) => {
  const rule = sla[step];
  if (!rule || !anchorIso) return null;
  return dueIsoFrom(anchorIso, rule);
};
var wasConducted = (x) => !!x.actualStart || !!x.attendanceClosedAt || ["conducted", "attendance_closed", "in_review", "closed"].includes(x.status);
var sessionDead = (x) => ["cancelled", "rescheduled", "closed"].includes(x.status);
var taggedTest = (...text) => text.some((t) => (t ?? "").trim().startsWith("ZZ TEST"));
var conductedAt = (x) => x.actualEnd ?? x.actualStart ?? (wasConducted(x) ? `${x.sessionDate}T00:00:00Z` : null);
function buildLdWork(data, sla) {
  const open = [];
  const closed = [];
  const sessions = data.sessions;
  const requestByCode = new Map(data.requests.map((r) => [r.id, r]));
  const actorOf = /* @__PURE__ */ new Map();
  for (const a of data.activity) {
    const step = a.type === "ld_conducted" ? "conducted" : a.type === "ld_attendance_closed" ? "attendance" : null;
    if (!step) continue;
    const key = `${a.entityId}|${step}`;
    if (!actorOf.has(key)) actorOf.set(key, { actorId: a.actorId, at: a.createdAt });
  }
  const logged = (sessionId, step) => actorOf.get(`${sessionId}|${step}`) ?? null;
  for (const e of buildQueueEntries4(data.requests, sla)) {
  ...
  for (const r of data.requests) {
    ...
    const put = (stepKey, actorId, doneAtIso, dueIso) => {
      if (!doneAtIso) return;
  ...
  for (const x of sessions) {
    ...
    const firstApproved = approvedNoms.reduce(
    ...
    const attended = new Set(
      (attBySession.get(x.id) ?? []).filter((a) => ["present", "partial"].includes(a.status)).map((a) => a.employeeId)
    );
  ...
  for (const v of data.submissions) {
    ...
    const x = a ? sessionById.get(a.sessionId) : void 0;
    if (!a || !x) continue;
    const dead = sessionDead(x);
    const pbase = {
      scope: "participant",
  ...
  const sessionsOfRequest = /* @__PURE__ */ new Map();
  for (const x of sessions) {
...
// frontend/src/core/workspace/mywork/items/learning-development.ts
var APPROVAL_STEPS2 = /* @__PURE__ */ new Set([
  "need_validation",
  "hr_head_approval",
  "mgmt_approval",
  "nomination_approval",
...
function learningDevelopmentWorkItems(data, uid, isAdmin) {
  const stepOwners = data.stepOwners ?? [];
  const { open } = buildLdWork(data, resolveStepSla4(data.config?.step_sla ?? null));
  const assignee = /* @__PURE__ */ new Map();
  ...
  return open.filter(isMine).map((o) => {
    ...
    return {
      ...
      ref: o.ref,
      detail: o.title,
      stage: stepByKey4(o.stepKey)?.short,
      dueIso: o.dueIso,
      to: o.sessionId ? `/learning-development/sessions/${o.sessionId}` : `/learning-development/requests/${o.entityId}`,
      /*
      ...
      isApproval: APPROVAL_STEPS2.has(o.stepKey)
    };
  });
}

// frontend/src/apps/fms-control-center/ranking/modules/learningDevelopment.ts
var label4 = (k) => stepByKey4(k)?.title ?? k;
var EXCLUDED2 = /* @__PURE__ */ new Set(["need_raised"]);
var learningDevelopmentScorer = {
  key: "learning-development",
  appId: "learning-development",
  load: () => fetchLdData(),
  closed(data) {
    const work = buildLdWork(data, resolveStepSla4(data.config?.step_sla ?? null));
    const out = [];
    for (const c of work.closed) {
      const drop = EXCLUDED2.has(c.stepKey) ? "excluded_step" : c.isTest ? "test_record" : !c.actorId ? "no_actor" : !c.doneAtIso ? "no_time" : void 0;
      out.push({
        stepId: c.stepId,
        entityId: c.entityId,
        ref: c.ref,
        stepKey: c.stepKey,
        stepLabel: label4(c.stepKey),
        roundNo: 0,
        dueIso: c.dueIso,
        actorId: c.actorId,
        /*
  ...
  openFor(data, uid) {
    const work = buildLdWork(data, resolveStepSla4(data.config?.step_sla ?? null));
    const flagOf = new Map(work.open.map((o) => [`${o.rowId}:${o.stepKey}`, o.isTest]));
    ...
    return parseItems(items).map(
      ({ entityId, stepKey, item }) => ({
        ...
        ref: item.ref,
        stepKey,
        stepLabel: label4(stepKey),
        roundNo: 0,
        dueIso: item.dueIso,
        drop: flagOf.get(`${entityId}:${stepKey}`) ? "test_record" : void 0
      })
    );
  }
};
...
async function fetchAll6(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE6) {
    const { data, error } = await db4.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE6 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
var num5 = (v) => v === null || v === void 0 ? null : Number(v);
var mapStepOwner3 = (r) => ({
  id: r.id,
  stepKey: r.step_key,
  departmentIds: r.department_ids ?? [],
...
var mapDeal = (r) => ({
  ...
  inkOfferQty: num5(r.ink_offer_qty),
  inkOfferRate: num5(r.ink_offer_rate),
  inkOfferSubtotal: num5(r.ink_offer_subtotal),
  ...
  headOfferQty: num5(r.head_offer_qty),
  headOfferRate: num5(r.head_offer_rate),
  headOfferSubtotal: num5(r.head_offer_subtotal),
  dryerType: r.dryer_type ?? null,
  dealValueCurrency: r.deal_value_currency ?? null,
  dealValueAmount: num5(r.deal_value_amount),
  fxRate: num5(r.fx_rate),
  fxRateAt: r.fx_rate_at ?? null,
  fxRateSource: r.fx_rate_source ?? null,
  fxRateOverridden: r.fx_rate_overridden ?? null,
  dealValueInr: num5(r.deal_value_inr),
  ...
  gstRate: num5(r.gst_rate),
  machineValueInr: num5(r.machine_value_inr),
  gstAmountInr: num5(r.gst_amount_inr),
  totalInr: num5(r.total_inr),
  postWarrantyHeadPrice: num5(r.post_warranty_head_price),
...
async function fetchOcpiData() {
  const [
    ...
    fetchAll6("fms_ocpi_step_owners"),
    fetchAll6("fms_ocpi_config", "key"),
    fetchAll6("fms_ocpi_company_profiles", "sort_order"),
    fetchAll6("fms_ocpi_machines", "sort_order"),
    fetchAll6("fms_ocpi_machine_sections", "sort_order"),
    fetchAll6("fms_ocpi_deals"),
    fetchAll6("fms_ocpi_quotation_versions", "generated_at"),
    fetchAll6("fms_ocpi_notifications"),
    fetchAll6("fms_ocpi_head_types", "sort_order"),
    fetchAll6("fms_ocpi_ink_types", "sort_order"),
    fetchAll6("fms_ocpi_dryer_types", "sort_order"),
    fetchAll6("fms_ocpi_master_managers"),
    fetchAll6("fms_ocpi_master_requests"),
    fetchAll6("fms_ocpi_machine_categories", "sort_order"),
    fetchAll6("fms_ocpi_dryers", "sort_order"),
    fetchAll6("fms_ocpi_machine_head_types", "sort_order"),
    fetchAll6("fms_ocpi_sales_pages", "sort_order")
  ]);
...
var stepByKey5 = (key) => STEPS6.find((s) => s.key === key);
var LIVE_STEPS = STEPS6.filter((s) => !s.retired);
var RETIRED_STEPS = STEPS6.filter((s) => s.retired).map((s) => s.key);
...
var model5 = createStepSlaModel(STEPS6, OVERRIDES3);
...
function stepCompletedIso2(d, step) {
  switch (step) {
    case "quotation":
...
function dueIsoFor2(d, step, stepSla) {
  const sla = stepSla?.[step];
  if (!sla) return null;
  return dueIsoFrom(stepCompletedIso2(d, sla.anchor) ?? d.createdAt, sla);
}
function buildQueueEntries5(deals, stepSla) {
  const out = [];
  for (const d of deals) {
    ...
    out.push({
      ...
      dealId: d.id,
      ref: dealRef(d),
      dueIso: stepSla ? dueIsoFor2(d, step, stepSla) : null,
...
var ownedEntries = perDataset((data) => {
  const sla = resolveStepSla5(data.stepSla);
  const byId = new Map(data.deals.map((d) => [d.id, d]));
  return buildQueueEntries5(data.deals, sla).map((e) => {
...
var ocpiScorer = {
  ...
  closed(data) {
    const sla = resolveStepSla5(data.stepSla);
    const openNow = new Map(buildQueueEntries5(data.deals, sla).map((e) => [e.dealId, e.stepKey]));
    const out = [];
    for (const d of data.deals) {
      for (const step of STEPS7) {
        const at = stepCompletedIso2(d, step);
        ...
        out.push({
          ...
          ref: dealRef(d),
          stepKey: step,
          stepLabel: label5(step),
          roundNo: 0,
          dueIso: dueIsoFor2(d, step, sla),
...
var model6 = createStepSlaModel(STEPS8, OVERRIDES4);
...
async function fetchAll7(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE7) {
    const { data, error } = await supabase.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE7 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchSuppliesData() {
  const [
    ...
    fetchAll7("fms_supplies_step_owners"),
    fetchAll7("fms_supplies_config", "key"),
    fetchAll7("designations"),
    fetchAll7("fms_supplies_companies"),
    fetchAll7("fms_supplies_departments"),
    fetchAll7("fms_supplies_categories"),
    fetchAll7("fms_supplies_items"),
    fetchAll7("fms_supplies_service_types"),
    fetchAll7("fms_supplies_master_managers"),
    fetchAll7("fms_supplies_master_requests"),
    fetchAll7("fms_supplies_requests", "submitted_at"),
    fetchAll7("fms_supplies_activity"),
    fetchAll7("fms_supplies_notifications")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
...
function heldStep2(r) {
  const def = stepByKey6(r.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
function buildHeldEntries3(snap) {
  const out = [];
  for (const r of snap.requests) {
    if (r.status !== "on_hold") continue;
    const step = heldStep2(r);
    if (!step) continue;
    out.push({
      stepKey: step,
      entityType: "request",
...
function officeSuppliesWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries6(snap).map((e) => ({ e, held: false })),
    ...buildHeldEntries3(snap).map((e) => ({ e, held: true }))
  ];
  return entries.filter(({ e }) => isAdmin || mine(e.stepKey, e.departmentId, e.assignedApproverId)).map(({ e, held }) => ({
...
var stepByKey7 = (key) => STEPS9.find((s) => s.key === key);
...
var OWNER_STEPS2 = [...STEPS9, SALES_RETURN_STEP];

// frontend/src/apps/order-to-dispatch/lib/sla.ts
var model7 = createStepSlaModel(STEPS9, {
...
async function fetchAll8(table, orderBy = "created_at", cols = "*") {
  return pagedWalk((withCount) => {
    let q = db5.from(table).select(cols, withCount ? { count: "exact" } : void 0).order(orderBy, { ascending: true });
...
async function fetchWhere(table, extra, cols = "*") {
  return pagedWalk(
    (withCount) => extra(db5.from(table).select(cols, withCount ? { count: "exact" } : void 0)).order("created_at", { ascending: true }).order("id", { ascending: true })
  );
}
...
var mapLots = (rows) => (Array.isArray(rows) ? rows : []).map((r) => ({ lotNo: String(r.lot_no ?? ""), qty: num6(r.qty), seq: Number(r.seq ?? 0) })).filter((l) => l.lotNo !== "").sort((a, b) => a.seq - b.seq);
var mapLine = (r) => ({
  id: r.id,
  orderId: r.order_id,
  lineNo: r.line_no,
...
async function fetchDispatchData(ctx) {
  ...
  const [
    ...
    fetchAll8("fms_dispatch_step_owners"),
    fetchAll8("fms_dispatch_step_assignees", "order_id"),
    fetchAll8("fms_dispatch_config", "key"),
    fetchAll8("designations"),
    fetchAll8("fms_dispatch_master_managers"),
    fetchAll8("fms_dispatch_master_requests"),
    fetchAll8("fms_dispatch_orders", "submitted_at"),
    fetchAll8("fms_dispatch_order_items", "created_at", LINE_COLS),
    fetchAll8("fms_dispatch_rounds", "archived_at"),
    fetchAll8("fms_dispatch_round_items", "created_at", RITEM_COLS),
    ...
    forUser ? fetchWhere("fms_dispatch_notifications", (q) => q.eq("user_id", forUser)) : fetchAll8("fms_dispatch_notifications")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
  ...
  const { data: orderPeek } = await db5.rpc("fms_dispatch_peek_order_no");
  return {
...
function heldStep3(o) {
  const def = stepByKey7(o.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
function buildHeldEntries4(snap) {
  const out = [];
  for (const o of snap.orders) {
    if (o.status !== "on_hold") continue;
    const step = heldStep3(o);
    if (!step) continue;
    out.push({
      stepKey: step,
      entityType: "order",
...
// frontend/src/apps/order-to-dispatch/lib/format.ts
var CREDIT_STATUS_LABEL = {
  approved: "Approved",
  partial: "Partially approved",
  credit_hold: "On hold"
};
var isCreditHeld = (o) => o.ccStatus === "credit_hold" && !o.ccAt;
var isBillHeld = (o) => !!o.sbHoldAt && !o.sbAt;
var STEP_HOLD = {
  credit_check: {
    held: isCreditHeld,
...
var STEP_STATUS = {
  ...
  sales_bill: (o) => isBillHeld(o) ? { label: STEP_HOLD.sales_bill.label, tone: "yellow", reason: o.sbHoldReason, rank: 0 } : null
};
...
function dispatchWorkItems(data, uid, isAdmin) {
  ...
  const stepParked = (stepKey, orderId) => {
    const rule = STEP_STATUS[stepKey];
  ...
  const entries = [
    ...buildQueueEntries7(snap).map((e) => ({ e, held: false })),
    ...buildHeldEntries4(snap).map((e) => ({ e, held: true }))
  ];
...
var STEPS10 = ["material_status", "sales_bill", "gate_out", "dispatch_confirm"];
var EXCLUDED3 = /* @__PURE__ */ new Set(["credit_check"]);
  ...
  for (const o of data.orders) {
    for (const v of allRoundViews(o)) {
      ...
      out.push({
        ...
        stepLabel: label7("credit_check"),
        roundNo: v.roundNo,
        dueIso,
        actorId,
        doneAtIso: atIso,
        drop: TEST_ORDERS.has(o.orderNo) ? "test_record" : void 0
      });
    }
  }
  return out;
}
var dropFor = (ref, stepKey) => TEST_ORDERS.has(ref) ? "test_record" : EXCLUDED3.has(stepKey) ? "excluded_step" : void 0;
  ...
  openFor(data, uid) {
    ...
    return parseItems(dispatchWorkItems(data, uid, false)).map(({ entityId, stepKey, item }) => {
      ...
      return {
        ...
        ref: item.ref,
        stepKey,
        stepLabel: stepKey === "sales_return" ? "Sales Return" : label7(stepKey),
        roundNo,
        dueIso: item.dueIso,
...
var mapRequest4 = (r) => ({
  ...
  requiredQty: num7(r.required_qty),
  ...
  bomLines: Array.isArray(r.bom_lines) ? r.bom_lines.map((l) => ({
    ...
    requiredQty: num7(l.required_qty),
  ...
  tsBomLines: Array.isArray(r.ts_bom_lines) ? r.ts_bom_lines.map((l) => ({
    ...
    unitId: l.unit_id ?? null,
    requestedQty: num7(l.requested_qty),
    handoverQty: num7(l.handover_qty),
    actualUse: num7(l.actual_use),
    lotNo: l.lot_no ?? null,
    isNew: !!l.is_new
  })) : [],
  tsPackedQty: num7(r.ts_packed_qty),
  tsLooseQty: num7(r.ts_loose_qty),
  tsProductionLoss: num7(r.ts_production_loss),
  ...
  peExpectedQty: num7(r.pe_expected_qty),
  actualQty: num7(r.actual_qty),
  scrapQty: num7(r.scrap_qty),
  peLabQty: num7(r.pe_lab_qty),
  ...
  pmhBomLines: Array.isArray(r.pmh_bom_lines) ? r.pmh_bom_lines.map((l) => ({
    ...
    qty: num7(l.qty),
    extra: num7(l.extra),
    extraBuffered: num7(l.extra_buffered),
    total: num7(l.total)
  })) : [],
...
async function fetchProductionData() {
  const [
    ...
    fetchAll9("fms_production_step_owners"),
    fetchAll9("fms_production_config", "key"),
    fetchAll9("designations"),
    fetchAll9("fms_production_categories"),
    fetchAll9("fms_production_raw_materials"),
    fetchAll9("fms_production_packaging_items"),
    fetchAll9("fms_production_fg_items"),
    fetchAll9("fms_production_units"),
    fetchAll9("fms_production_test_equipments"),
    fetchAll9("fms_production_coa_parameters"),
    fetchAll9("fms_production_coas"),
    fetchAll9("fms_production_boms"),
    fetchAll9("fms_production_bom_components"),
    fetchAll9("fms_production_master_managers"),
    fetchAll9("fms_production_master_requests"),
    fetchAll9("fms_production_requests", "submitted_at"),
    fetchAll9("fms_production_activity"),
    fetchAll9("fms_production_notifications"),
    db6.rpc("fms_production_peek_batch_no")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
...
function heldStep4(r) {
  const def = stepByKey8(r.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
function buildHeldEntries5(snap) {
  const out = [];
  for (const r of snap.requests) {
    if (r.status !== "on_hold") continue;
    const step = heldStep4(r);
    if (!step) continue;
    out.push({
      stepKey: step,
      entityType: "request",
...
var QUEUE_STEPS = STEPS11.filter((s) => !s.noQueue).map((s) => s.key);
var TITLE = Object.fromEntries(STEPS11.map((s) => [s.key, s.title]));
var SHORT = Object.fromEntries(STEPS11.map((s) => [s.key, s.short]));
...
function productionWorkItems(data, uid, isAdmin) {
  ...
  const reasonById = new Map(data.requests.map((r) => [r.id, r.holdReason]));
  const entries = [
    ...buildQueueEntries8(snap).map((e) => ({ e, held: false })),
    ...buildHeldEntries5(snap).map((e) => ({ e, held: true }))
  ];
  return entries.filter(({ e }) => isAdmin || isMineByStepOwners(e.stepKey, uid, owners)).map(({ e, held }) => ({
...
var model9 = createStepSlaModel(STEPS12);
...
async function fetchAll10(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE10) {
    const { data, error } = await supabase.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE10 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchAllOptional2(table, orderBy = "created_at") {
  try {
    return await fetchAll10(table, orderBy);
  } catch (e) {
    const msg = e.message ?? "";
    if (/does not exist|schema cache/i.test(msg)) return [];
    throw e;
...
var num8 = (v) => v === null || v === void 0 ? null : Number(v);
var mapRequest5 = (r) => ({
  id: r.id,
  requestNo: r.request_no,
  companyId: r.company_id,
...
var mapRequestItem2 = (r) => ({
  ...
  sourcingReason: r.sourcing_reason ?? null,
  finalVendorId: r.final_vendor_id ?? null,
  finalQty: num8(r.final_qty),
  finalRate: num8(r.final_rate),
  gstPct: num8(r.gst_pct),
  leadTimeDays: r.lead_time_days ?? null,
  lineValue: num8(r.line_value),
  status: r.status,
  approverId: r.approver_id ?? null,
  assignedApproverId: r.assigned_approver_id ?? null,
  approvalTier: r.approval_tier ?? null,
...
async function fetchProcurementData() {
  const [
    ...
    fetchAll10("designations"),
    fetchAll10("fms_purchase_step_owners"),
    fetchAll10("fms_purchase_approval_matrix"),
    fetchAll10("fms_purchase_config", "key"),
    fetchAll10("fms_purchase_requests"),
    fetchAll10("fms_purchase_request_items"),
    fetchAll10("fms_purchase_request_vendors"),
    fetchAll10("fms_purchase_vendor_item_prices"),
    fetchAll10("fms_purchase_quotations"),
    fetchAll10("fms_purchase_pos"),
    fetchAll10("fms_purchase_po_items"),
    fetchAll10("fms_purchase_pis"),
    fetchAll10("fms_purchase_pi_items"),
    fetchAll10("fms_purchase_grns"),
    fetchAll10("fms_purchase_grn_items"),
    fetchAll10("fms_purchase_tally_bookings"),
    fetchAll10("fms_purchase_qc_inspections"),
    fetchAll10("fms_purchase_qc_items"),
    fetchAll10("fms_purchase_payments"),
  ...
  const config = {
    ...
    // Unset or partially-stored rules fall back to the code defaults.
    stepSla: resolveStepSla9(configByKey.get("step_sla")),
    reassignPoolDepartmentIds: configByKey.get("reassign_pool")?.department_ids ?? [],
    reassignPoolUserIds: configByKey.get("reassign_pool")?.user_ids ?? []
  };
  return {
...
var slaFor2 = (data, step) => data.config?.stepSla?.[step] ?? DEFAULT_STEP_SLA9[step] ?? { anchor: step, days: 1 };
...
var lineInApproval3 = (l) => l.status === "approval" || l.status === "on_hold";
...
var requestInApproval = (idx, r) => linesOf(idx, r.id).some(lineInApproval3);
var requestApprovalTotal2 = (idx, requestId) => linesOf(idx, requestId).filter(lineInApproval3).reduce((sum, l) => sum + (l.lineValue ?? 0), 0);
...
var REQUEST_STEP_MATCH = {
  ...
  approval: lineInApproval3,
  po: lineInPoDesk2
};
function requestDueIso(data, idx, r, step) {
  const match = REQUEST_STEP_MATCH[step] ?? lineInApproval3;
...
function buildQueueEntries9(data, idx = buildProcIndex(data)) {
  const out = [];
  for (const r of data.requests) {
...
function purchaseWorkItems(data, uid, isAdmin) {
  ...
  const heldLines = (stepKey, entityId) => {
    if (stepKey !== "approval") return null;
    const underDecision = linesOf(idx, entityId).filter(lineInApproval3);
    if (underDecision.length === 0) return null;
    return underDecision.every((l) => l.status === "on_hold") ? underDecision : null;
  };
  return buildQueueEntries9(data).filter((e) => isAdmin || owners.isMine(e, uid)).map((e) => ({
    ...
    assignment: owners.isMine(e, uid) ? "direct" : "team",
    isApproval: e.stepKey === "approval",
    ...holdFieldsOf2(heldLines(e.stepKey, e.entityId))
  }));
}
function holdFieldsOf2(held) {
  if (!held) return {};
...
async function fetchSamplingData() {
  const [
    ...
    fetchAll11("fms_sampling_step_owners"),
    fetchAll11("fms_sampling_step_source_owners"),
    fetchAll11("fms_sampling_config", "key"),
    fetchAll11("designations"),
    fetchAll11("fms_sampling_companies"),
    fetchAll11("fms_sampling_collectors"),
    fetchAll11("fms_sampling_handover_recipients"),
    fetchAll11("fms_sampling_senders"),
    fetchAll11("fms_sampling_confirmers"),
    fetchAll11("fms_sampling_result_recipients"),
    fetchAll11("fms_sampling_master_managers"),
    fetchAll11("fms_sampling_requests", "submitted_at"),
    fetchAll11("fms_sampling_activity"),
    fetchAll11("fms_sampling_notifications")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
    stepSla: resolveStepSla10(byKey.get("step_sla"))
  };
  return {
    ...
    requests: requests.map(mapRequest6),
...
function heldStep5(r) {
  const def = stepByKey10(r.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
function buildHeldEntries6(snap) {
  const out = [];
  for (const r of snap.requests) {
    if (r.status !== "on_hold") continue;
    const step = heldStep5(r);
    if (!step) continue;
    out.push({
      stepKey: step,
      entityType: "request",
...
function buildQueueEntries10(snap) {
  const out = [];
  for (const r of snap.requests) {
...
function samplingWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries10(snap).map((e) => ({ e, held: false })),
    ...buildHeldEntries6(snap).map((e) => ({ e, held: true }))
  ];
  return entries.filter(
...
function mapPolicy(raw) {
  const p = raw ?? {};
  return {
    maxPassengers: num9(p.max_passengers, POLICY_DEFAULTS.maxPassengers),
    bookingWindowDays: num9(p.booking_window_days, POLICY_DEFAULTS.bookingWindowDays),
    advanceBookingWarnDays: num9(p.advance_booking_warn_days, POLICY_DEFAULTS.advanceBookingWarnDays),
    claimDeadlineDays: num9(p.claim_deadline_days, POLICY_DEFAULTS.claimDeadlineDays),
    claimHardStopDays: num9(p.claim_hard_stop_days, POLICY_DEFAULTS.claimHardStopDays),
    advanceMaxPct: num9(p.advance_max_pct, POLICY_DEFAULTS.advanceMaxPct),
    advanceRecoveryDays: num9(p.advance_recovery_days, POLICY_DEFAULTS.advanceRecoveryDays),
    hodReviewDays: num9(p.hod_review_days, POLICY_DEFAULTS.hodReviewDays),
    financeProcessDays: num9(p.finance_process_days, POLICY_DEFAULTS.financeProcessDays),
    creditDays: num9(p.credit_days, POLICY_DEFAULTS.creditDays),
    disputeThreshold: num9(p.dispute_threshold, POLICY_DEFAULTS.disputeThreshold),
    hotelCapHardMultiple: num9(p.hotel_cap_hard_multiple, POLICY_DEFAULTS.hotelCapHardMultiple),
    emergencyWindowHours: num9(p.emergency_window_hours, POLICY_DEFAULTS.emergencyWindowHours)
  };
}
function mapMatrix(raw) {
  const m = raw ?? {};
  return {
    directorFromBand: num9(m.director_from_band, 6),
...
async function fetchTravelData() {
  const [
    ...
    fetchAll12("fms_travel_passengers", "id"),
    ...
    fetchAll12("fms_travel_legs", "id"),
    fetchAll12("fms_travel_step_owners"),
    fetchAll12("fms_travel_step_assignees", "trip_id"),
    fetchAll12("fms_travel_config", "key"),
    fetchAll12("fms_travel_employee_settings"),
    fetchAll12("fms_travel_notifications"),
    fetchAll12("fms_travel_cities", "sort_order"),
    fetchAll12("fms_travel_purposes", "sort_order"),
    fetchAll12("fms_travel_expense_categories", "sort_order"),
    fetchAll12("fms_travel_airlines", "sort_order"),
    fetchAll12("fms_travel_hotels", "sort_order"),
    fetchAll12("fms_travel_bus_operators", "sort_order"),
    fetchAll12("fms_travel_rate_cards", "effective_from"),
    fetchAll12("fms_travel_rates", "sort_order"),
    fetchAll12("fms_travel_master_managers"),
    fetchAll12("fms_travel_master_requests"),
    ...
    fetchAll12("fms_travel_claim_lines", "id"),
    fetchAll12("fms_travel_da_days", "id")
  ]);
  const cfg = new Map(configRows.map((r) => [r.key, r.value]));
...
var stepByKey11 = (key) => STEPS14.find((s) => s.key === key);
var OWNER_STEPS3 = STEPS14.map((s) => s.key);
...
var model11 = createStepSlaModel(STEPS14, OVERRIDES6);
var DEFAULT_STEP_SLA11 = model11.DEFAULT_STEP_SLA;
var anchorOptions11 = model11.anchorOptions;
...
function buildQueueEntries11(trips, stepSla) {
  const out = [];
...
function buildHeldEntries7(trips, stepSla) {
  const out = [];
  for (const t of trips) {
    if (t.status !== "on_hold") continue;
    const def = t.currentStep ? stepByKey11(t.currentStep) : void 0;
...
var APPROVAL_STEPS4 = /* @__PURE__ */ new Set([
  "manager_approval",
  "director_approval",
  "claim_review",
  "finance_review"
...
function travelDeskWorkItems(data, uid, isAdmin) {
  const stepOwners = data.stepOwners ?? [];
  const stepSla = resolveStepSla11(data.stepSla);
  const assigneeByKey = new Map(
  ...
  const entries = [
    ...buildQueueEntries11(data.trips, stepSla).map((e) => ({ e, held: false })),
    ...buildHeldEntries7(data.trips, stepSla).map((e) => ({ e, held: true }))
  ];
  return entries.filter(({ e }) => isAdmin || isMine(e)).map(({ e, held }) => ({
    id: `travel-desk:${e.entityId}:${e.stepKey}`,
    source: "travel-desk",
    sourceLabel: appName("travel-desk"),
    ref: e.ref,
    detail: e.travellerName,
    stage: stepByKey11(e.stepKey)?.short,
    dueIso: e.dueIso,
    ...
    */
    ...
    isApproval: APPROVAL_STEPS4.has(e.stepKey),
...
var label11 = (k) => stepByKey11(k)?.title ?? k;
...
var travelDeskScorer = {
  ...
  closed(data) {
    const sla = resolveStepSla11(data.stepSla);
    const out = [];
    for (const t of data.trips) {
      const selfClosed = !!t.stAt && t.crAt === t.stAt && t.frAt === t.stAt;
      for (const step of STEPS15) {
        const at = stepCompletedIso3(t, step);
        if (!at) continue;
  ...
  openFor(data, uid) {
    return parseItems(travelDeskWorkItems(data, uid, false)).map(
      ({ entityId, stepKey, item }) => ({
        ...
        ref: item.ref,
        stepKey,
        stepLabel: label11(stepKey),
        roundNo: 0,
        dueIso: item.dueIso,
        drop: heldDrop(item) ?? (isTestTrip(item.ref) ? "test_record" : void 0)
      })
    );
  }
};
...
// supabase/kpi/entry.ts
var moduleNameOf = (module) => module === TASK_MODULE ? appName(TASK_MODULE) : appName(RANKED_MODULES[module]?.appId ?? module);
async function moduleFacts(module, staffIds) {
  const t0 = Date.now();
  if (module === TASK_MODULE) {
    const input = await loadTaskFactsInput();
    const t12 = Date.now();
    const out2 = taskFacts(input);
    return { ...out2, ms: { load: t12 - t0, compute: Date.now() - t12 } };
  }
  ...
  const open = openSteps(data, staffIds).get(module) ?? /* @__PURE__ */ new Map();
