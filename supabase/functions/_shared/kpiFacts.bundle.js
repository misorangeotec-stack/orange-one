...
async function fetchAll5(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE5) {
    const { data, error } = await db4.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE5 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchAll8(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE8) {
    const { data, error } = await db6.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE8 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchAll11(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE11) {
    const { data, error } = await db8.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE11 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
