...
function heldStep2(r) {
  const def = stepByKey5(r.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
...
function officeSuppliesWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries5(snap).map((e) => ({ e, held: false })),
...
function heldStep3(o) {
  const def = stepByKey6(o.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
...
var STEPS9 = ["material_status", "sales_bill", "gate_out", "dispatch_confirm"];
...
function heldStep4(r) {
  const def = stepByKey7(r.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
...
function heldStep5(r) {
  const def = stepByKey9(r.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
...
function samplingWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries9(snap).map((e) => ({ e, held: false })),
...
function buildHeldEntries7(trips, stepSla) {
  ...
  for (const t of trips) {
    ...
    const def = t.currentStep ? stepByKey10(t.currentStep) : void 0;
...
function travelDeskWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries10(data.trips, stepSla).map((e) => ({ e, held: false })),
