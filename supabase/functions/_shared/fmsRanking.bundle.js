...
async function fetchHrData() {
  ...
  const [
    ...
    fetchAll2("fms_hr_probation_checkins", "created_at"),
    fetchAll2("fms_hr_buddies", "created_at"),
    fetchAll2("fms_hr_buddy_interactions", "created_at"),
...
async function fetchAll4(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE4) {
    const { data, error } = await supabase.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE4 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchLdData() {
  const [
    ...
    fetchAll4("fms_ld_step_owners"),
    fetchAll4("fms_ld_step_assignees", "assigned_at"),
    fetchAll4("fms_ld_config", "key"),
    fetchAll4("fms_ld_session_types", "sort_order"),
    fetchAll4("fms_ld_competencies", "sort_order"),
    fetchAll4("fms_ld_need_sources", "sort_order"),
    fetchAll4("fms_ld_venues", "sort_order"),
    fetchAll4("fms_ld_trainers", "sort_order"),
    fetchAll4("fms_ld_delay_reasons", "sort_order"),
    fetchAll4("fms_ld_followup_actions", "sort_order"),
    fetchAll4("fms_ld_master_managers"),
    fetchAll4("fms_ld_master_requests"),
    fetchAll4("fms_ld_requests"),
    fetchAll4("fms_ld_sessions"),
    fetchAll4("fms_ld_activity"),
    fetchAll4("fms_ld_notifications"),
    fetchAll4("fms_ld_nominations", "nominated_at"),
    fetchAll4("fms_ld_materials", "uploaded_at"),
    fetchAll4("fms_ld_attendance", "marked_at"),
    fetchAll4("fms_ld_assignments", "issued_at"),
    // ⚠ NOT the default "created_at" — this table has none. See the note on fetchAll.
    fetchAll4("fms_ld_assignment_submissions", "updated_at"),
    fetchAll4("fms_ld_feedback", "submitted_at"),
    fetchAll4("fms_ld_effectiveness", "due_on"),
    fetchAll4("fms_ld_plans"),
    fetchAll4("fms_ld_plan_lines", "planned_month"),
    fetchAll4("fms_ld_mandatory_programs", "sort_order")
  ]);
  const cfg = {};
...
async function fetchAll5(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE5) {
    const { data, error } = await db3.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE5 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchOcpiData() {
  const [
    ...
    fetchAll5("fms_ocpi_step_owners"),
    fetchAll5("fms_ocpi_config", "key"),
    fetchAll5("fms_ocpi_company_profiles", "sort_order"),
    fetchAll5("fms_ocpi_machines", "sort_order"),
    fetchAll5("fms_ocpi_machine_sections", "sort_order"),
    fetchAll5("fms_ocpi_deals"),
    fetchAll5("fms_ocpi_quotation_versions", "generated_at"),
    fetchAll5("fms_ocpi_notifications"),
    fetchAll5("fms_ocpi_head_types", "sort_order"),
    fetchAll5("fms_ocpi_ink_types", "sort_order"),
    fetchAll5("fms_ocpi_dryer_types", "sort_order"),
    fetchAll5("fms_ocpi_master_managers"),
    fetchAll5("fms_ocpi_master_requests"),
    fetchAll5("fms_ocpi_machine_categories", "sort_order"),
    fetchAll5("fms_ocpi_dryers", "sort_order"),
    fetchAll5("fms_ocpi_machine_head_types", "sort_order"),
    fetchAll5("fms_ocpi_sales_pages", "sort_order")
  ]);
...
async function fetchAll6(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE6) {
    const { data, error } = await supabase.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE6 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function pagedWalk(build) {
  const page = (from, withCount = false) => build(withCount).range(from, from + PAGE7 - 1);
  const first = await page(0, true);
  if (first.error) throw new Error(first.error.message);
  const out = [...first.data ?? []];
  if (out.length < PAGE7) return out;
  const total = first.count ?? null;
  const known = total === null ? 0 : Math.ceil(total / PAGE7);
  if (known > 1) {
    const rest = await Promise.all(
      Array.from({ length: known - 1 }, (_, i) => page((i + 1) * PAGE7))
    );
    for (const r of rest) {
      if (r.error) throw new Error(r.error.message);
      out.push(...r.data ?? []);
    }
    if ((rest[rest.length - 1]?.data?.length ?? 0) < PAGE7) return out;
  }
  for (let from = Math.max(known, 1) * PAGE7; ; from += PAGE7) {
...
async function fetchAll7(table, orderBy = "created_at", cols = "*") {
...
async function fetchDispatchData(ctx) {
  ...
  const [
    ...
    fetchAll7("fms_dispatch_step_owners"),
    fetchAll7("fms_dispatch_step_assignees", "order_id"),
    fetchAll7("fms_dispatch_config", "key"),
    fetchAll7("designations"),
    fetchAll7("fms_dispatch_master_managers"),
    fetchAll7("fms_dispatch_master_requests"),
    fetchAll7("fms_dispatch_orders", "submitted_at"),
    fetchAll7("fms_dispatch_order_items", "created_at", LINE_COLS),
    fetchAll7("fms_dispatch_rounds", "archived_at"),
    fetchAll7("fms_dispatch_round_items", "created_at", RITEM_COLS),
    ...
    forUser ? fetchWhere("fms_dispatch_notifications", (q) => q.eq("user_id", forUser)) : fetchAll7("fms_dispatch_notifications")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
...
async function fetchAll8(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE8) {
    const { data, error } = await db5.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE8 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchProductionData() {
  const [
    ...
    fetchAll8("fms_production_test_equipments"),
    fetchAll8("fms_production_coa_parameters"),
    fetchAll8("fms_production_coas"),
...
async function fetchAll9(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE9) {
    const { data, error } = await supabase.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE9 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchAllOptional2(table, orderBy = "created_at") {
  try {
    return await fetchAll9(table, orderBy);
  } catch (e) {
    const msg = e.message ?? "";
    if (/does not exist|schema cache/i.test(msg)) return [];
    throw e;
...
async function fetchProcurementData() {
  const [
    ...
    fetchAll9("fms_purchase_companies"),
    fetchAll9("fms_purchase_categories"),
    fetchAll9("fms_purchase_item_groups"),
    fetchAll9("fms_purchase_items"),
    fetchAll9("fms_purchase_vendors"),
    fetchAll9("fms_purchase_master_managers"),
    fetchAll9("fms_purchase_master_requests"),
    fetchAll9("fms_purchase_po_cancel_requests"),
    fetchAll9("designations"),
    fetchAll9("fms_purchase_step_owners"),
    fetchAll9("fms_purchase_approval_matrix"),
    fetchAll9("fms_purchase_config", "key"),
    fetchAll9("fms_purchase_requests"),
    fetchAll9("fms_purchase_request_items"),
    fetchAll9("fms_purchase_request_vendors"),
    fetchAll9("fms_purchase_vendor_item_prices"),
    fetchAll9("fms_purchase_quotations"),
    fetchAll9("fms_purchase_pos"),
    fetchAll9("fms_purchase_po_items"),
    fetchAll9("fms_purchase_pis"),
    fetchAll9("fms_purchase_pi_items"),
    fetchAll9("fms_purchase_grns"),
    fetchAll9("fms_purchase_grn_items"),
    fetchAll9("fms_purchase_tally_bookings"),
    fetchAll9("fms_purchase_qc_inspections"),
    fetchAll9("fms_purchase_qc_items"),
    fetchAll9("fms_purchase_payments"),
    fetchAll9("fms_purchase_followups"),
    fetchAll9("fms_purchase_activity"),
    fetchAll9("fms_purchase_notifications"),
...
async function fetchAll10(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE10) {
    const { data, error } = await db6.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE10 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
async function fetchSamplingData() {
  const [
    ...
    fetchAll10("fms_sampling_step_owners"),
    fetchAll10("fms_sampling_step_source_owners"),
    fetchAll10("fms_sampling_config", "key"),
    fetchAll10("designations"),
    fetchAll10("fms_sampling_companies"),
    fetchAll10("fms_sampling_collectors"),
    fetchAll10("fms_sampling_handover_recipients"),
    fetchAll10("fms_sampling_senders"),
    fetchAll10("fms_sampling_confirmers"),
    fetchAll10("fms_sampling_result_recipients"),
    fetchAll10("fms_sampling_master_managers"),
    fetchAll10("fms_sampling_requests", "submitted_at"),
    fetchAll10("fms_sampling_activity"),
    fetchAll10("fms_sampling_notifications")
  ]);
  const byKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: byKey.get("process_coordinators")?.user_ids ?? [],
...
async function fetchTravelData() {
  const [
    ...
    fetchAll11("fms_travel_passengers", "id"),
    ...
    fetchAll11("fms_travel_legs", "id"),
    fetchAll11("fms_travel_step_owners"),
    fetchAll11("fms_travel_step_assignees", "trip_id"),
    fetchAll11("fms_travel_config", "key"),
    fetchAll11("fms_travel_employee_settings"),
    fetchAll11("fms_travel_notifications"),
    fetchAll11("fms_travel_cities", "sort_order"),
    fetchAll11("fms_travel_purposes", "sort_order"),
    fetchAll11("fms_travel_expense_categories", "sort_order"),
    fetchAll11("fms_travel_airlines", "sort_order"),
    fetchAll11("fms_travel_hotels", "sort_order"),
    fetchAll11("fms_travel_bus_operators", "sort_order"),
    fetchAll11("fms_travel_rate_cards", "effective_from"),
    fetchAll11("fms_travel_rates", "sort_order"),
    fetchAll11("fms_travel_master_managers"),
    fetchAll11("fms_travel_master_requests"),
    ...
    fetchAll11("fms_travel_claim_lines", "id"),
    fetchAll11("fms_travel_da_days", "id")
  ]);
  const cfg = new Map(configRows.map((r) => [r.key, r.value]));
