...
function holdAwareBucketOf(item, todayIso2) {
  if (item.isHeld) return "hold";
  return bucketOf(item.dueIso, todayIso2);
}
...
async function fetchAllOptional2(table, orderBy = "created_at") {
  try {
    return await fetchAll5(table, orderBy);
  } catch (e) {
    const msg = e.message ?? "";
    if (/does not exist|schema cache/i.test(msg)) return [];
    throw e;
...
async function fetchImportData() {
  const [
    ...
    fetchAllOptional2("fms_import_sourcing_docs")
  ]);
  const configByKey = new Map(configRows.map((r) => [r.key, r.value ?? {}]));
  const config = {
    processCoordinatorIds: configByKey.get("process_coordinators")?.user_ids ?? [],
...
async function fetchDispatchData(ctx) {
  ...
  const [
    ...
    fetchAll9("fms_dispatch_order_items", "created_at", LINE_COLS),
    fetchAll9("fms_dispatch_rounds", "archived_at"),
    fetchAll9("fms_dispatch_round_items", "created_at", RITEM_COLS),
...
async function fetchTravelData() {
  ...
  return {
    ...
    stepAssignees: assigneeRows.map(mapStepAssignee4),
    config,
    stepSla: cfg.get("step_sla") ?? null,
    ...
    masterManagers: managerRows.map(mapMasterManager7),
...
var stepByKey10 = (key) => STEPS10.find((s) => s.key === key);
...
var model10 = createStepSlaModel(STEPS10, OVERRIDES5);
...
async function fetchAll12(table, orderBy = "created_at") {
  ...
  for (let from = 0; ; from += PAGE11) {
    const { data, error } = await supabase.from(table).select("*").order(orderBy, { ascending: true }).range(from, from + PAGE11 - 1);
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    out.push(...rows);
...
var num9 = (v) => v === null || v === void 0 ? null : Number(v);
var master = (r) => ({
  id: r.id,
  name: r.name,
  active: r.active ?? true,
...
var mapRequest6 = (r) => ({
  ...
  proposedCost: num9(r.proposed_cost),
  ...
  approvedBudget: num9(r.approved_budget),
...
async function fetchLdData() {
  const [
    ...
    fetchAll12("fms_ld_step_owners"),
    fetchAll12("fms_ld_step_assignees", "assigned_at"),
    fetchAll12("fms_ld_config", "key"),
    fetchAll12("fms_ld_session_types", "sort_order"),
    fetchAll12("fms_ld_competencies", "sort_order"),
    fetchAll12("fms_ld_need_sources", "sort_order"),
    fetchAll12("fms_ld_venues", "sort_order"),
    fetchAll12("fms_ld_trainers", "sort_order"),
    fetchAll12("fms_ld_delay_reasons", "sort_order"),
    fetchAll12("fms_ld_followup_actions", "sort_order"),
    fetchAll12("fms_ld_master_managers"),
    fetchAll12("fms_ld_master_requests"),
    fetchAll12("fms_ld_requests"),
    fetchAll12("fms_ld_sessions"),
    fetchAll12("fms_ld_activity"),
    fetchAll12("fms_ld_notifications"),
    fetchAll12("fms_ld_nominations", "nominated_at"),
    fetchAll12("fms_ld_materials", "uploaded_at"),
    fetchAll12("fms_ld_attendance", "marked_at"),
    fetchAll12("fms_ld_assignments", "issued_at"),
    // ⚠ NOT the default "created_at" — this table has none. See the note on fetchAll.
    fetchAll12("fms_ld_assignment_submissions", "updated_at"),
    fetchAll12("fms_ld_feedback", "submitted_at"),
    fetchAll12("fms_ld_effectiveness", "due_on"),
    fetchAll12("fms_ld_plans"),
    fetchAll12("fms_ld_plan_lines", "planned_month"),
    fetchAll12("fms_ld_mandatory_programs", "sort_order")
  ]);
  const cfg = {};
  ...
  return {
    ...
    planLines: planLines.map((r) => ({
      ...
      plannedHours: num9(r.planned_hours),
      estimatedCost: num9(r.estimated_cost),
      note: r.note,
      sortOrder: r.sort_order ?? 0
    })),
    ...
    stepSla: resolveStepSla10(cfg.step_sla ?? null),
...
function importWorkItems(data, uid, isAdmin) {
  ...
  const heldCount = (stepKey, entityId) => {
    if (stepKey !== "approval") return null;
    const underDecision = data.requestItems.filter((l) => l.requestId === entityId && lineInApproval2(l));
    if (underDecision.length === 0) return null;
  ...
  return buildQueueEntries2(data).filter((e) => isAdmin || owners.isMine(e, uid)).map((e) => ({
    ...
    assignment: owners.isMine(e, uid) ? "direct" : "team",
    isApproval: e.stepKey === "approval",
    ...holdFieldsOf2(heldCount(e.stepKey, e.entityId))
  }));
}
function holdFieldsOf2(n) {
  if (n === null) return {};
...
function hrWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries3(snap).map((e) => ({ e, held: false })),
...
function stepsByProgress(c, checks, skipped = NO_SKIPS) {
...
function collectEntries(snap, pick) {
  const out = [];
...
function buildHeldEntries2(snap) {
  return collectEntries(
    snap,
    (c, checks, skipped) => c.status === "on_hold" ? stepsByProgress(c, checks, skipped) : []
  );
}
...
function hrExitWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...buildQueueEntries4(snap).map((e) => ({ e, held: false })),
    ...buildHeldEntries2(snap).map((e) => ({ e, held: true }))
  ];
  return entries.filter(({ e }) => isAdmin || ownersOf(e, stepOwners, assigneeByKey).includes(uid)).map(({ e, held }) => ({
...
function heldStep4(o) {
  const def = stepByKey8(o.currentStep);
  if (!def || def.noQueue) return null;
  return def.key;
}
...
function heldStep5(j) {
  const def = j.currentStep ? stepByKey9(j.currentStep) : void 0;
  if (!def || def.noQueue) return null;
  return def.key;
}
function buildHeldEntries7(snap) {
  const out = [];
  for (const j of snap.jobs) {
    if (j.status !== "on_hold") continue;
    const step = heldStep5(j);
    if (!step) continue;
    out.push({
      stepKey: step,
      entityType: "job",
...
function assetWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...
    ...buildHeldEntries7(snap).map((e) => ({ e, held: true }))
  ];
...
var stepByKey11 = (key) => STEPS11.find((s) => s.key === key);
var OWNER_STEPS2 = STEPS11.map((s) => s.key);
...
var INERT_STEPS = STEPS11.filter((s) => s.noQueue).map((s) => s.key);
var model11 = createStepSlaModel(STEPS11, OVERRIDES6);
...
function tripDueIso(t, step, stepSla) {
  ...
  const trigger = TRIGGER_STEPS2[step];
  if (trigger) {
    if (!from) return null;
...
function buildQueueEntries10(trips, stepSla) {
  ...
  for (const t of trips) {
    const step = STATUS_STEP4[t.status];
    if (!step) continue;
    out.push({
      stepKey: step,
      entityId: t.id,
...
function buildHeldEntries8(trips, stepSla) {
  const out = [];
  for (const t of trips) {
...
function travelDeskWorkItems(data, uid, isAdmin) {
  ...
  const entries = [
    ...
    ...buildHeldEntries8(data.trips, stepSla).map((e) => ({ e, held: true }))
  ];
...
function buildQueueEntries11(requests, sla) {
  const out = [];
...
function buildLdWork(data, sla) {
  ...
  for (const e of buildQueueEntries11(data.requests, sla)) {
...
var APPROVAL_STEPS5 = /* @__PURE__ */ new Set([
  "need_validation",
  "hr_head_approval",
  "mgmt_approval",
  "nomination_approval",
...
function learningDevelopmentWorkItems(data, uid, isAdmin) {
  const stepOwners = data.stepOwners ?? [];
  const { open } = buildLdWork(data, resolveStepSla10(data.config?.step_sla ?? null));
  ...
  return open.filter(isMine).map((o) => {
    ...
    return {
      ...
      ref: o.ref,
      detail: o.title,
      stage: stepByKey10(o.stepKey)?.short,
      dueIso: o.dueIso,
      ...
      isApproval: APPROVAL_STEPS5.has(o.stepKey)
    };
  });
}

// frontend/src/apps/universal.ts
var UNIVERSAL_APP_IDS = [
  "kra-kpi",
  /*
...
function computeSnapshot(data, userId, isAdmin, appIds, maxItems = 40) {
  ...
  const tiles = { overdue: 0, dueToday: 0, next2: 0, noDate: 0, hold: 0 };
  const per = /* @__PURE__ */ new Map();
  for (const item of scoped) {
    const b = holdAwareBucketOf(item, today);
