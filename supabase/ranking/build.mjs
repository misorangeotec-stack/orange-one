 ...
 *
 * THE SECOND BUNDLE — the KRA / KPI facts (KPI-1)
 *   The `kpi-facts` edge function asks every module the same two questions through the
 *   same scorers, so it needs the same substitutions, the same browser-code guard and the
 *   same "every FMS is scored or excused" rule. It is built by THIS script rather than a
 *   copy of it, so the two can never be shimmed differently:
 *
 *     node supabase/ranking/build.mjs kpi  →  supabase/functions/_shared/kpiFacts.bundle.js
 *
 *   With no argument the script builds the ranking exactly as before.
 */
...
const TARGETS = {
  ranking: { entry: resolve(here, "entry.ts"), out: "fmsRanking.bundle.js", fn: "fms-ranking" },
  kpi: { entry: resolve(repo, "supabase/kpi/entry.ts"), out: "kpiFacts.bundle.js", fn: "kpi-facts" },
};
const target = process.argv[2] ?? "ranking";
if (!TARGETS[target]) throw new Error(`unknown bundle "${target}" — expected one of: ${Object.keys(TARGETS).join(", ")}`);

const ENTRY = TARGETS[target].entry;
...
const OUT = resolve(repo, "supabase/functions/_shared", TARGETS[target].out);
...
if (found.length) {
  throw new Error(
    `${TARGETS[target].fn} bundle contains browser-only code: ${found.join(", ")}.\n` +
...
await writeFile(
  OUT,
  `// GENERATED by supabase/ranking/build.mjs${target === "ranking" ? "" : ` ${target}`} — do not edit by hand.\n` +
