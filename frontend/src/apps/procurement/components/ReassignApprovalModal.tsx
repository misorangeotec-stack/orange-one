 ...
 *
 * The handover MOVES the work: the requisition leaves the amount band's queue and
 ...
 * approver to the band in Setup, which would share every requisition in that band
 * rather than pass one.
 *
 * Three things are deliberate, the first two learned from the version of this
 * feature that was removed on the Import side
 * (20260806123000_fms_import_remove_reassign.sql):
 *
 *   1. The picker offers ONLY the people an admin named in Setup, plus the
 *      members of THIS requisition's band so it can be handed back. The removed
 *      version listed every profile in the company, which is why it was dropped.
 ...
 *   3. The candidate list is per-REQUISITION, not a flat "all approvers": Purchase
 *      routes by amount band, so a name from another band could not receive this
 *      one and fms_purchase_reassign_request would refuse it.
 */
export default function ReassignApprovalModal({
  ...
  const options: ComboOption[] = useMemo(
    () => (request ? s.reassignCandidates(request).map((c) => ({ value: c.id, label: c.name })) : []),
    [s, request]
  );
      ...
      <div className="space-y-3.5">
        <div className="rounded-xl bg-page px-3.5 py-2.5">
          ...
          <p className="text-[13px] font-semibold text-navy">
            {holder ? s.personName(holder) : "The approvers on this requisition's amount band"}
          </p>
        </div>
        ...
        {empty ? (
          <p className="rounded-lg bg-[#FEF6E0] px-3 py-2 text-[12.5px] leading-snug text-[#946200]">
            ...
            <Link to="/procurement/settings" className="font-semibold underline">
              Setup &rarr; Approval Matrix
            </Link>
            .
          </p>
        ) : (
