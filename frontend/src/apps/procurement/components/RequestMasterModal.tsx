 ...
 * typed, plus whatever parent the form already knows) or standalone from the
 * Master Requests page with the type picker unlocked.
 *
 * The request routes to that master's assigned managers, or to the admins when
 * none are assigned; the reviewer can edit the payload before it lands.
 */
export default function RequestMasterModal({
  ...
  lockType?: boolean;
  /** e.g. `{ name: "ACME", category_id: "<the form's category>" }`. */
  prefill?: MasterValues;
  /** This modal opens on top of another one (e.g. inside the Sourcing dialog). */
  stacked?: boolean;
  onRequested?: (id: string, masterType: MasterType, name: string) => void;
}) {
  const s = useProcurementStore();
  const [mt, setMt] = useState<MasterType | null>(masterType);
