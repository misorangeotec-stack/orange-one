...
import { SharePoModal, AddPiModal, PaymentModal, FollowupModal, GrnModal, TallyModal, QcModal, PurchaseReturnModal, GateOutwardModal } from "../../components/PoModals";
import type { PurchaseOrder, Pi, Payment, Followup, Grn, QcInspection, TallyBooking } from "../../types";
...
function StepQueuePage<E>({
  ...
  // Company as a sortable/filterable column for the Completed tab too (StageEntry
  // rows carry a build-time companyId). The group band is hidden below.
  const companyColumn: QueueColumn<StageEntry<E>> = {
    key: "company",
    header: "Company",
    cell: (e) => companyName(e.companyId ?? ""),
    sortValue: (e) => companyName(e.companyId ?? ""),
    filter: { kind: "select", get: (e) => companyName(e.companyId ?? "") },
      ...
      <Card className="p-4">
        {showingCompleted ? (
          <QueueTable
            rows={completedRows}
            rowKey={(e) => e.id}
            columns={[companyColumn, ...completed.columns]}
...
export function CollectPiQueue() {
  ...
  const piColumns: QueueColumn<StageEntry<Pi>>[] = [
    ...
    { key: "piStatus", header: "Status", cell: (e) => <span className={`${PILL} text-grey-2 bg-page`}>{e.row.status.replace(/_/g, " ")}</span>, sortValue: (e) => e.row.status, filter: { kind: "select", get: (e) => e.row.status.replace(/_/g, " ") }, tdClassName: "whitespace-nowrap", resize: false /* a pill: never cut (PF-20) */ },
  ...
  return (
    <>
      <StepQueuePage<QcInspection>
        ...
        filter={(p) => poInPurchaseReturn(s.procIndex, p)}
        completed={{
  ...
  return (
    <>
      <StepQueuePage<QcInspection>
        ...
        filter={(p) => poInGateOutward(s.procIndex, p)}
        completed={{
