...
import { HR_MASTER_TYPES, type HrMasterType } from "../../types";

/**
 * Master Owners (admin only) — who may open each HR master. An owner can CRUD
 * that master and resolve its new-entry requests; a request for a master with no
 * owner assigned falls back to the admins. Writes replace the whole set for a type.
 */
export default function MasterOwnersSection() {
  const s = useHrStore();
  const [editing, setEditing] = useState<HrMasterType | null>(null);
    ...
    <Card className="overflow-hidden">
      <div className="px-4 pt-4 pb-1">
        <p className="text-[12.5px] text-grey">
          Each master's owner reviews the "request a new entry" submissions raised from the recruitment forms — they can
          approve (adjusting the details first), or reject with a reason. Leave a master unassigned and its requests go
          to the admins. The Onboarding Checklist can be owned too, but it feeds no dropdown, so it is never requested —
          its owner just edits it on the Masters page.
        </p>
      </div>
      ...
      <Modal
        ...
        title={`Owners — ${HR_MASTER_TYPES.find((m) => m.value === editing)?.plural ?? ""}`}
