 ...
 *  • Inert (`mrf`) — raising the requisition IS the event; it never waits in a
 *    queue, so no due date is computed. It exists only as an anchor for later steps.
 *  • Trigger-anchored (the five probation steps) — anchored on the joining date,
 *    and counted in calendar MONTHS, not working days. The "Due after" cell is
 *    static; the number stays editable.
    ...
    <div className="space-y-4">
          ...
          <table className="w-full text-[13.5px]">
            ...
            <tbody>
              {STEPS.map((st) => {
                ...
                const months = trigger?.unit === "months";
                    ...
                    <td className="px-4 py-3">
                      {inert ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          {months && <span className="text-[12px] text-grey-2 whitespace-nowrap">months</span>}
                        </div>
                      )}
                    </td>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="ml-auto text-[12px] text-grey-2">
          Working days run Monday to Saturday — Sundays are skipped. Probation reviews count calendar months from the
          joining date.
        </span>
      </div>
    </div>
