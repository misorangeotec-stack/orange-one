...
import { fetchEmailModuleEnabled, setEmailModuleEnabled, HR_MODULE_ID } from "../../data/emailSettings";

const QK = ["emailModuleSetting", HR_MODULE_ID];

/**
 * Admin: the per-module email ON/OFF switch for New Recruitment.
 *
 * NARROWER than the purchase-family apps' equivalent, and the copy says so:
 * fms_hr_announce only enqueues email for MASTER REQUEST events, so this
 * governs master-data governance mail alone. Recruitment step alerts (CVs
 * added, shared with HOD, offer confirmation) stay bell-only whatever this is
 * set to. The in-app bell is never affected either way.
 */
export default function EmailNotificationsSection() {
  const s = useHrStore();
  const canConfigure = s.canConfigure;
  const qc = useQueryClient();
  const { data, isLoading } = useQuery({ queryKey: QK, queryFn: () => fetchEmailModuleEnabled(HR_MODULE_ID) });
  ...
  const mut = useMutation({
    mutationFn: (v: boolean) => setEmailModuleEnabled(HR_MODULE_ID, v),
    onSuccess: async () => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-[12.5px] text-grey-2 mt-2 leading-relaxed">
            Recruitment step alerts — CVs added, shared with HOD, offer confirmation and the rest — stay in-app only and
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none">
          ...
          <span>
            <span className="block text-[13.5px] font-medium text-navy">
              Email master requests for New Recruitment
            </span>
