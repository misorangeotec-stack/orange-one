...
import { HR_MASTER_TYPES, type HrMasterRequest } from "../types";
import {
  describePayload,
  masterFields,
  masterTypeLabel,
 ...
 * proposed values first if needed, or reject with a reason. Everyone else gets
 * "My requests" — what they've asked for and where each one stands. Anyone can
 * raise a new one from here, for any requestable master.
 */
export default function MasterRequests() {
  ...
  const [approving, setApproving] = useState<HrMasterRequest | null>(null);
  const [rejecting, setRejecting] = useState<HrMasterRequest | null>(null);
  ...
  const unassigned = HR_MASTER_TYPES.filter((m) => s.isMasterUnassigned(m.value));

  const openApprove = (r: HrMasterRequest) => {
    setValues({ ...(r.proposedPayload as MasterValues) });
    setErr(null);
    setApproving(r);
  };
  const openReject = (r: HrMasterRequest) => {
    setNote("");
    setErr(null);
    setRejecting(r);
  };
  ...
  const statusBadge = (st: HrMasterRequest["status"]) => {
    const map = {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[13.5px] text-grey-2 mt-1">
            {canReview
              ? "New-master-entry requests raised from across recruitment. Approve to add them to the master — adjusting the details first if you need to — or reject with a reason."
              : "Entries you've asked to add to the recruitment masters. Once the master's owner approves one, it's selectable on the forms."}
          </p>
        </div>
