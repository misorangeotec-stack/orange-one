...
/**
 * The Production Control Center — the SAME queue entries every page counts,
 * bucketed by due date. Built through `snapshotFrom(store.queueEntries, STEPS,
 * STAGES)`, exactly as the cross-FMS scoreboard adapter does.
 */
export default function ControlCenter() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-[22px] font-bold text-navy">{appName("production-entry")} Control Center</h1>
        <p className="text-[13.5px] text-grey-2 mt-1">Open work across the whole production floor, by how close it is to its due date.</p>
      </div>
...
function StageRows({ label, counts, steps }: { label: string; counts: Record<Bucket, number>; steps: { stepKey: string; label: string; counts: Record<Bucket, number> }[] }) {
  return (
    <>
      <tr className="bg-navy/[0.03] border-b border-line">
        <td className="px-4 py-2.5 font-semibold text-navy">{label}</td>
        {BUCKETS.map((b) => (
          <td key={b.key} className="px-4 py-2.5 text-center font-semibold text-navy">{counts[b.key] || <span className="text-grey-2">—</span>}</td>
        ))}
      </tr>
      {steps.map((st) => (
        <tr key={st.stepKey} className="border-b border-line/70 last:border-0">
          ...
          {BUCKETS.map((b) => (
            <td key={b.key} className="px-4 py-2.5 text-center text-grey">{st.counts[b.key] || <span className="text-grey-2">—</span>}</td>
          ))}
        </tr>
      ))}
    </>
