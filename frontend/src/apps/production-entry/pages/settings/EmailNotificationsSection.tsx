...
import { fetchEmailModuleEnabled, setEmailModuleEnabled, PRODUCTION_ENTRY_MODULE_ID } from "../../data/emailSettings";

const QK = ["emailModuleSetting", PRODUCTION_ENTRY_MODULE_ID];

/**
 * Admin: the per-module email ON/OFF switch for Production Entry. Email only flows
...
export default function EmailNotificationsSection() {
  ...
  const { data, isLoading } = useQuery({ queryKey: QK, queryFn: () => fetchEmailModuleEnabled(PRODUCTION_ENTRY_MODULE_ID) });
  ...
  const mut = useMutation({
    mutationFn: (v: boolean) => setEmailModuleEnabled(PRODUCTION_ENTRY_MODULE_ID, v),
    onSuccess: async () => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-[12.5px] text-grey-2 mt-1 leading-relaxed">
            When on, each Production Entry step also emails the next responsible person(s) — the same people who get the
            in-app alert (each step of the job card hands off to the next: handover → transfer slip → production →
            quality → testing → packing → FG transfer, and FG transfer closes the job card to the requester). Emails are
            sent from support@orangeotec.com. In-app alerts are unaffected.
          </p>
        </div>
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none">
          ...
          />
          <span>
            <span className="block text-[13.5px] font-medium text-navy">Send email notifications for Production Entry</span>
