...
/**
 * Process coordinators (admin). They see the Control Center, can act on any step,
 * and can hold job cards. Stored in fms_production_config under `process_coordinators`.
 */
export default function CoordinatorsSection() {
  ...
  const save = async () => {
    ...
    try { await s.setCoordinators(picked); setSaved(true); }
  ...
  return (
    <Card className="p-5 space-y-4 max-w-xl">
      <p className="text-[12.5px] text-grey">
        Coordinators oversee the whole production process — they can act on any step, hold a job card, and open the Control Center.
      </p>
