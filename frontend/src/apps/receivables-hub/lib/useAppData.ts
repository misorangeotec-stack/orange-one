...
// consolidateByName + the risk banding live in ./appDataCore (pure, React-free) so the
// scheduled-report runner can rebuild a report outside the browser off the same code.
// Re-exported here because every page already imports consolidateByName from useAppData.
export { consolidateByName, categorizeRisk } from "./appDataCore";
import { consolidateByName, categorizeRisk } from "./appDataCore";
...
interface AppData {
   ...
   * silently jumps back to gross the moment someone picks a sale type.
   */
...
export function useAppData(filters: Filters = {}): AppData {
  ...
  const kpis = useMemo<KPIs | null>(() => {
    ...
    // Only the two scalars belong on KPIs — `onAccountByCustomer` is a lookup for the top-risky
    // list, not a headline figure, and spreading it here would not type-check.
    const { totalOverdueOnBills, totalOverdueCreditsApplied, onAccountByCustomer } = overdueBridge;
    return {
      totalOverdueOnBills,
      totalOverdueCreditsApplied,
