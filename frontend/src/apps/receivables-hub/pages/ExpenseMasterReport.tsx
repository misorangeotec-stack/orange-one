...
import {
  ...
  PieChart as PieIcon, RefreshCw, TrendingDown, TrendingUp, Wallet, type LucideIcon,
} from "lucide-react";
...
import {
  fmtExp, tickExp, pctChange, expPeriod, expFyOptions, priorFy, expCat,
  loadExpenseReport, loadExpenseLastRefresh, refreshExpenseCompany, groupStats, movers,
  EXP_CURRENT, EXP_PRIOR,
  type GroupStat, type ExpGroupRow, type ExpenseFilters,
} from "@hub/lib/expenseReport";

/**
 * Master Reports → Finance → Expense.
 *
 * A faithful rebuild of the Talligence "Master Reports – Finance – Expense" screen
 * (Misc/Talligence-Inputs/Master Reports - Finance - Expense.pdf) in Orange One's palette:
 * the four mover KPIs (Expense / Highest Change / Highest Increase / Lowest Change), the
 * Quarterly + Trend + Groups + Sub-Groups charts, and the Group → Sub-group → Ledger table.
 *
 * Every figure comes from ONE ConnectWave RPC (`rpt_expense_report`) reading the precomputed
 * rpt_expense_line snapshot. Expense = net posted to expense-group ledgers {Direct Expenses,
 * Indirect Expenses, Purchase Accounts}, debit-positive — the sign-mirror of the Income report.
 * Reconciled for Orange O Tec Noida FY2026-27: total PYTD ₹8.3440 Cr / PY ₹25.8557 Cr, Direct
 * ₹-11.08 L / ₹-7.96 L, Indirect PYTD ₹75.85 L, Purchase ₹7.6963 Cr / ₹23.1433 Cr — every PY and
 * PYTD figure ties to Talligence. The CY column deliberately does not: Talligence reads ₹4.2 Cr
 * against the books' ₹6.98 Cr because its current-year sync is stale (the same staleness proven
 * when the Purchase Report was built). We ship Tally-true.
 *
 * TONE: unlike Income, a RISE here is bad. The change pills and the Change (in %) column are
 * inverted — up is red, down is green — while the arrow still follows the arithmetic sign.
 */
 ...
 * Colour is INVERTED versus the Income report: spending more is the bad outcome.
 */
...
/* ---- Hero banner (matches the Sales / Payables / Income reports) ---------- */
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h1 className="mt-1.5 flex items-center gap-2 text-[24px] font-bold tracking-tight sm:text-[27px]">
            <Wallet className="h-6 w-6 text-orange" /> Expense
          </h1>
...
/* ---- KPI cards (faithful Talligence: Expense / Highest Change / Increase / Lowest) --- */
...
/**
 * A change pill "1.95 % ▲". Inverted versus Income — a rise in expense is red.
 * forceUp shows +100% for a dormant (0→0) group, as Talligence does.
 */
function ChangePill({ pct, forceUp }: { pct: number | null; forceUp?: boolean }) {
  ...
  const up = shown >= 0;
  return (
    <span className={cn("inline-flex items-center gap-0.5 text-[12.5px] font-semibold", up ? "text-ryg-red" : "text-emerald-600")}>
...
export default function ExpenseMasterReport() {
  const { companies, loading: coLoading, error: coError } = useFinancialStatements();
  const [params, setParams] = useSearchParams();

  const fyOptions = useMemo(() => expFyOptions(), []);
  ...
  const period = useMemo(() => expPeriod(fy), [fy]);
  const prior = priorFy(fy);

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["expenseReport", "v1", companyGuid, fy, filters],
    queryFn: () => loadExpenseReport(companyGuid, fy, filters),
    enabled: !!companyGuid,
    staleTime: 5 * 60 * 1000,
  });

  const { data: lastRefresh, refetch: refetchLast } = useQuery({
    queryKey: ["expenseLastRefresh", companyGuid, fy],
    queryFn: () => loadExpenseLastRefresh(companyGuid, fy),
    enabled: !!companyGuid,
    staleTime: 60 * 1000,
  });
  ...
  const onRefresh = async () => {
    ...
    try {
      const res = await refreshExpenseCompany(companyGuid, fy);
      if (res.status === "cooldown") {
        ...
        setRefreshNote(`Refreshed in ${res.seconds}s — ${res.lines ?? 0} expense lines.`);
  ...
  // Clamped at 0: a pie cannot render a negative slice, and expense months genuinely can be
  // negative (a purchase-return-heavy month, or Direct Expenses running net credit).
  ...
  // Sub-groups chart reads `subgroup_bars`, NOT `subgroups`: Talligence charts the immediate child
  // of the primary group, which is the ledger when a ledger hangs straight off it (so Purchase
  // Accounts shows GST BRANCH PURCHASE-INK / GST PURCHASE rather than one "Purchase Accounts" bar).
  const subgroupBars = useMemo(
    () => (data?.subgroup_bars ?? []).map((s) => ({ name: s.sub, cy: Number(s.ytd), py: Number(s.py) })),
    [data],
  );
  const subgroupHasNeg = subgroupBars.some((b) => b.cy < 0 || b.py < 0);

  // Table rows: only the expense groups that carry an amount (the source table omits dormant ones).
  ...
  const heroSummary: React.ReactNode = heroLoading
    ? "Gathering the expense book…"
    : data
      ? (
        <>
          <span className="font-semibold text-white">{fmtExp(data.kpi.ytd)}</span> of expense booked so far this year
          {growthPct != null && (
            <>, {growthPct >= 0 ? "up" : "down"}{" "}
      ...
      : "Expense posted to the Direct & Indirect Expense and Purchase Accounts groups — straight from the Tally books.";
  ...
  const heroMeta = [
    ...
    "Currency ₹",
    "Auto-refreshes daily at 9:30 PM",
  ].join("   ·   ");
  ...
  const hintFor = (g: ExpGroupRow) => `PYTD ${fmtExp(g.pytd)} · PY ${fmtExp(g.py)}`;
    ...
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto space-y-3">
      ...
      {busy && (
        <div className="space-y-1">
          ...
          <div className="text-[11px] text-muted-foreground">
            Rebuilding this company's expense snapshot — {elapsed.toFixed(0)}s elapsed
      ...
      <Card className="rounded-card border-border bg-surface shadow-sm p-3">
        <div className="flex flex-wrap items-end gap-3">
          {([
            ["Expense Group", "groups", data?.filters.groups],
      ...
      {errText ? (
        ...
        <div className="py-16 text-center text-muted-foreground text-sm">Loading the expense book…</div>
      ) : !data ? (
        <div className="py-16 text-center text-muted-foreground text-sm">
          No expense snapshot for this company yet — press Refresh to build it.
        </div>
      ) : (
        <>
          {/* ── KPI row ──────────────────────────────────────────────── */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <KpiCard tone="orange" icon={Wallet} label="Expense" subtitle="Yearly"
                     value={fmtExp(data.kpi.ytd)} pct={pctChange(data.kpi.ytd, data.kpi.pytd)}
                     hint={`PYTD ${fmtExp(data.kpi.pytd)} · PY ${fmtExp(data.kpi.py_total)}`} />
            <KpiCard tone="grey" icon={BarChart3} label="Highest Change"
                     subtitle={mv.highestChange?.grp ?? "—"}
                     value={mv.highestChange ? fmtExp(mv.highestChange.ytd) : "—"}
                     pct={mv.highestChange?.change ?? null}
                     hint={mv.highestChange ? hintFor(mv.highestChange) : ""} />
            {/* Rising expense is the bad outcome, so this card carries the red tone. */}
            <KpiCard tone="down" icon={TrendingUp} label="Highest Increase"
                     subtitle={mv.highestIncrease?.grp ?? "—"}
                     value={mv.highestIncrease ? fmtExp(mv.highestIncrease.ytd) : "—"}
                     pct={mv.highestIncrease?.change ?? null} forceUp
                     hint={mv.highestIncrease ? hintFor(mv.highestIncrease) : ""} />
            {/* Smallest rupee movement — the steady line, hence the calm tone. */}
            <KpiCard tone="up" icon={TrendingDown} label="Lowest Change"
                     subtitle={mv.lowestChange?.grp ?? "—"}
                     value={mv.lowestChange ? fmtExp(mv.lowestChange.ytd) : "—"}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <SalesPanel title="Quarterly Expense" icon={PieIcon} subtitle="Inner ring: current FY · Outer: prior FY">
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={quarterly.cy} dataKey="value" nameKey="name" innerRadius={30} outerRadius={62}>
                    {quarterly.cy.map((_, i) => <Cell key={i} fill={expCat(i)} />)}
                  </Pie>
                  <Pie data={quarterly.py} dataKey="value" nameKey="name" innerRadius={72} outerRadius={100}>
                    {quarterly.py.map((_, i) => <Cell key={i} fill={expCat(i)} fillOpacity={0.45} />)}
                  </Pie>
                  <Tooltip formatter={(v: number) => fmtExp(v)} />
                  <Legend verticalAlign="bottom" height={24} iconSize={9} wrapperStyle={{ fontSize: 11 }} />
                </PieChart>
              </ResponsiveContainer>
            </SalesPanel>

            <SalesPanel title="Expense Trend" icon={BarChart3} subtitle="Monthly expense, this FY vs last">
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={monthly}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} vertical={false} />
                  <XAxis dataKey="name" tick={{ ...AXIS_TICK, fontSize: 10 }} interval={0} />
                  <YAxis tickFormatter={tickExp} tick={AXIS_TICK} width={46} />
                  <Tooltip formatter={(v: number) => fmtExp(v)} />
                  <Legend iconSize={9} wrapperStyle={{ fontSize: 11 }} />
                  <Bar dataKey="py" name={`FY ${prior}`} fill={EXP_PRIOR} radius={[3, 3, 0, 0]} />
                  <Bar dataKey="cy" name={`FY ${fy}`} fill={EXP_CURRENT} radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </SalesPanel>
          </div>

          {/* ── Expense Groups · Sub Groups ──────────────────────────── */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <SalesPanel title="Expense Groups" icon={Layers} subtitle="By top-level group — current FY (YTD) vs prior FY">
            ...
            <SalesPanel title="Expense Sub Groups" icon={Layers} subtitle="By sub-group — purchase returns net negative">
              <ResponsiveContainer width="100%" height={260}>
          ...
          {/* ── Expense table (Group → Sub-group → Ledger) ───────────── */}
          <SalesPanel title="Expense" icon={Wallet} subtitle="Drill: group → sub-group → ledger" bodyClassName="p-0">
            <ScrollableTable className="border-b border-border">
              <table className="w-full">
                <thead className="bg-muted/40 border-b border-border">
                  <tr>
                    <Th>Expense Name</Th><Th right>YTD Amount</Th><Th right>PYTD Amount</Th>
                    ...
                    return (
                      <Fragment key={g.grp}>
                        <tr className="hover:bg-muted/30 cursor-pointer font-semibold"
                          ...
                          <Td right>{fmtExp(g.ytd)}</Td>
                          <Td right>{fmtExp(g.pytd)}</Td>
                          ...
                          return (
                            <Fragment key={subKey(g.grp, s.sub)}>
                              <tr className="hover:bg-muted/30 cursor-pointer bg-muted/10"
                                ...
                                <Td right>{fmtExp(s.ytd)}</Td>
                                <Td right>{fmtExp(s.pytd)}</Td>
                              ...
                              {sOpen && ledgersBySub(g.grp, s.sub).map((l) => (
                                <tr key={`${g.grp}|${s.sub}|${l.ledger}`} className="hover:bg-muted/30">
                                  ...
                                  <Td right>{fmtExp(l.ytd)}</Td>
                                  <Td right>{fmtExp(l.pytd)}</Td>
                ...
                <tfoot className="bg-muted/40 border-t border-border font-semibold">
                  <tr>
                    <Td>Total Expense</Td>
                    <Td right>{fmtExp(data.kpi.ytd)}</Td>
                    <Td right>{fmtExp(data.kpi.pytd)}</Td>
