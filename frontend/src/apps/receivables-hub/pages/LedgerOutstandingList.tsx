/**
 * Ledger Outstandings — the ledger list (screen 1). Every ledger across the loaded companies, with a
 * company filter, a Tally-group filter, and search. Click a ledger to open its pending bills, exactly
 * as Tally's "List of Ledgers → Ledger Outstandings" works.
 *
 * Mirrors the Risk Register's list scaffolding (filter bar + FilterChips + hand-rolled 25/page
 * pagination) rather than TallyReportFrame, which is company-card oriented.
 */
...
export default function LedgerOutstandingList() {
  ...
  // (a sub-group) not just "Current Assets" (its primary group). A group's top-level is the last
  // element of any chain it appears in; sub-groups are labelled with that primary group so they read
  // clearly and stay searchable ("sundry", "debtors", or "current" all find it).
  const groupOptions = useMemo(() => {
    const topOf = new Map<string, string>(); // group name → its primary (top-level) group
    for (const l of all) {
    ...
    <div className="p-6 space-y-5 max-w-[1400px] mx-auto">
      <div>
        ...
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
          <ReceiptText className="h-6 w-6 text-primary" /> Ledger Outstandings
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          Pick a ledger to see its pending bills, exactly as Tally shows them.
        </p>
      </div>
            ...
            <table className="w-full border-collapse min-w-[720px]">
              <thead>
                <tr className="border-b-2 border-border bg-muted/50">
                  ...
                  <th className="text-right py-2 px-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Outstanding</th>
                  <th className="w-8" />
                </tr>
              </thead>
              <tbody>
