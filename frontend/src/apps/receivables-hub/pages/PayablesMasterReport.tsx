...
import {
  ...
  Percent, ReceiptText, RefreshCw, ShoppingCart, Users, Wallet, type LucideIcon,
} from "lucide-react";
...
import {
  fmtPay, loadPayablesLastRefresh, loadPayablesReport, payFyOptions, payPeriod,
  refreshPayablesCompany, tickPay, PAY_CURRENT,
  type PartyRow, type PayablesFilters, type PayBillRow,
} from "@hub/lib/payablesReport";

/**
 * Master Reports → Finance → Payables.
 *
 * A faithful rebuild of the Talligence "Master Report – Finance – Payables" screen in
 * Orange One's palette — the sign-mirror of the Receivables report, reading Sundry Creditors:
 * the six KPI tiles, Monthly Purchase + Accounts Payables bars, the bill-age Ageing Balance,
 * the Party-Wise Bills Payables table and the Bill Details table. Every figure comes from ONE
 * ConnectWave RPC (`rpt_payables_report`).
 *
 * Reconciled for Orange O Tec Noida FY2026-27: Outstanding −₹18.45 L (net) and On Account
 * ₹5.90 L tie to Talligence exactly. Bills Payable / Overdue / Advance are the Tally-consistent
 * figures (the source report's Advance is a stale phantom — see payablesReport.ts).
 */
...
const AP_FILL = "#9957DB"; // violet — Accounts Payables, distinct from the orange purchase bar
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h1 className="mt-1.5 flex items-center gap-2 text-[24px] font-bold tracking-tight sm:text-[27px]">
            <HandCoins className="h-6 w-6 text-orange" /> Payables
          </h1>
...
export default function PayablesMasterReport() {
  const { companies, loading: coLoading, error: coError } = useFinancialStatements();
  const [params, setParams] = useSearchParams();

  const fyOptions = useMemo(() => payFyOptions(), []);
  ...
  const period = useMemo(() => payPeriod(fy), [fy]);
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["payablesReport", "v1", companyGuid, fy, filters],
    queryFn: () => loadPayablesReport(companyGuid, fy, filters),
    enabled: !!companyGuid,
    staleTime: 5 * 60 * 1000,
  });

  const { data: lastRefresh, refetch: refetchLast } = useQuery({
    queryKey: ["payablesLastRefresh", companyGuid, fy],
    queryFn: () => loadPayablesLastRefresh(companyGuid, fy),
    enabled: !!companyGuid,
    staleTime: 60 * 1000,
  });
  ...
  const onRefresh = async () => {
    ...
    try {
      const res = await refreshPayablesCompany(companyGuid, fy);
      if (res.status === "cooldown") {
        ...
        setRefreshNote("This is a large book — its snapshot is rebuilt by the nightly job (9:00 PM).");
      } else {
        setRefreshNote(`Refreshed in ${res.seconds}s — ${res.bills ?? 0} open bills across ${res.ledgers ?? 0} suppliers.`);
  ...
  const apSeries = useMemo(() => {
    const m = new Map<number, number>();
    for (const r of data?.accounts_payable ?? []) m.set(Number(r.ym.slice(4, 6)), Number(r.amt) || 0);
    return FY_MONTHS.map((mm) => ({ name: MONTH_LABEL[mm], v: m.get(mm) ?? 0 }));
  }, [data]);
  ...
  const partyTotals = useMemo(
    () => parties.reduce(
      ...
      { payable: 0, on_account: 0, advance: 0, outstanding: 0 },
    ),
    [parties],
  );

  const bills: PayBillRow[] = useMemo(() => {
  ...
  const heroSummary: React.ReactNode = heroLoading
    ? "Gathering the payables book…"
    : data
      ? (
        <>
          <span className="font-semibold text-white">{fmtPay(data.kpi.bills_payable)}</span> owed on open bills, of which{" "}
          <span className="font-semibold text-white">{fmtPay(data.kpi.overdue)}</span> is overdue
          {data.kpi.overdue_pct != null && <> ({Number(data.kpi.overdue_pct).toFixed(1)}%)</>}.
        </>
      )
      : "What we owe every supplier — outstanding, overdue, on-account and advances, straight from the Tally books.";
  ...
  const heroMeta = [
    ...
    "Currency ₹",
    "Auto-refreshes daily at 9:00 PM",
  ].join("   ·   ");
  ...
  return (
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto space-y-3">
      ...
      {busy && (
        <div className="space-y-1">
          ...
          <div className="text-[11px] text-muted-foreground">
            Rebuilding this company's payables snapshot — {elapsed.toFixed(0)}s elapsed
      ...
      {errText ? (
        ...
        <div className="py-16 text-center text-muted-foreground text-sm">Loading the payables book…</div>
      ) : !data ? (
        <div className="py-16 text-center text-muted-foreground text-sm">
          No payables snapshot for this company yet — press Refresh to build it.
        </div>
      ) : (
        <>
          {/* ── KPI row ──────────────────────────────────────────────── */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <KpiCard tone="orange" icon={Wallet} label="Outstanding"
                     value={fmtPay(data.kpi.outstanding)} hint="Net owed to suppliers (Cr)" />
            <KpiCard tone="slate" icon={ReceiptText} label="Bills Payables"
                     value={fmtPay(data.kpi.bills_payable)} hint="Open bills, gross" />
            <KpiCard tone="down" icon={CalendarClock} label="Overdue"
                     value={fmtPay(data.kpi.overdue)} hint="Past due date" />
            ...
            <KpiCard tone="slate" icon={HandCoins} label="On Account"
                     value={fmtPay(data.kpi.on_account)} hint="Unallocated debits" />
            <KpiCard tone="grey" icon={BadgeIndianRupee} label="Advance"
                     value={fmtPay(data.kpi.advance)} hint="Advance / debit bills" />
          </div>

          {/* ── Monthly Purchase · Accounts Payables ─────────────────── */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <SalesPanel title="Monthly Purchase" icon={ShoppingCart} subtitle="Billed purchases by month, this FY">
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={monthlyPurchase}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} vertical={false} />
                  <XAxis dataKey="name" tick={{ ...AXIS_TICK, fontSize: 10 }} interval={0} />
                  <YAxis tickFormatter={tickPay} tick={AXIS_TICK} width={46} />
                  <Tooltip formatter={(v: number) => fmtPay(v)} />
                  <Bar dataKey="v" name="Purchase" fill={PAY_CURRENT} radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </SalesPanel>

            <SalesPanel title="Accounts Payables" icon={Wallet} subtitle="Month-end payable balance"
                        empty={apSeries.every((r) => r.v === 0)}
                        emptyMessage="No month-end payable history for this company.">
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={apSeries}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} vertical={false} />
                  <XAxis dataKey="name" tick={{ ...AXIS_TICK, fontSize: 10 }} interval={0} />
                  <YAxis tickFormatter={tickPay} tick={AXIS_TICK} width={46} />
                  <Tooltip formatter={(v: number) => fmtPay(v)} />
                  <Bar dataKey="v" name="Payable" fill={AP_FILL} radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </SalesPanel>
          </div>
          ...
          {/* ── Ageing Balance ───────────────────────────────────────── */}
          <SalesPanel title="Ageing Balance" icon={Clock} subtitle="Open payable bills, bucketed by bill age"
          ...
          {/* ── Party Wise Bills Payables ────────────────────────────── */}
          <SalesPanel
            title="Party Wise Bills Payables"
            icon={Users}
            actions={
              <Input value={partyQuery} onChange={(e) => setPartyQuery(e.target.value)}
                     placeholder="Search supplier…" className="h-8 w-52 text-xs" />
            }
            bodyClassName="p-0"
            empty={parties.length === 0}
            emptyMessage="No suppliers with a balance."
          >
            <ScrollableTable className="border-b border-border">
              <table className="w-full">
                <thead className="bg-muted/40 border-b border-border">
                  <tr>
                    <Th>Vendor</Th>
                    <Th right>Payables Formula (in days)</Th>
                    <Th right>Actual Bill Clearance (in days)</Th>
                    <Th right>Payable Amount</Th>
                ...
                <tbody className="divide-y divide-border/60">
                  {partyPage.pageItems.map((p) => (
                    <tr key={p.ledger} className="hover:bg-muted/30">
                      ...
                      <Td right>{nDays(p.days_payable)}</Td>
                      <Td right>{nDays(p.days_clearance)}</Td>
                      <Td right>{fmtPay(p.payable)}</Td>
                      <Td right>{fmtPay(p.on_account)}</Td>
                      <Td right>{fmtPay(p.advance)}</Td>
                      <Td right>{fmtPay(p.outstanding)}</Td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="bg-muted/40 border-t border-border font-semibold">
                  <tr>
                    ...
                    <Td right>{fmtPay(partyTotals.payable)}</Td>
                    <Td right>{fmtPay(partyTotals.on_account)}</Td>
                    <Td right>{fmtPay(partyTotals.advance)}</Td>
                    <Td right>{fmtPay(partyTotals.outstanding)}</Td>
                  </tr>
                </tfoot>
              </table>
            </ScrollableTable>
            <Pagination state={partyPage} rowsLabel="suppliers" />
          </SalesPanel>
                ...
                <tbody className="divide-y divide-border/60">
                  {billPage.pageItems.map((b) => (
                    <tr key={`${b.ledger}|${b.bill_ref}`} className="hover:bg-muted/30">
                      ...
                      <Td right>{fmtPay(b.amount)}</Td>
                      <Td right>{fmtPay(b.pending)}</Td>
