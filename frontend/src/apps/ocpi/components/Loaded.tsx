 ...
 *   say "No deals yet" / "Nothing awaiting approval". That is not a slow page, it
 *   is a page telling you something FALSE: a salesperson with six quotations in
 ...
 * looks merely empty. This module carries customer-facing documents, so "no
 * quotations" must never be a rendering artifact.
 */
  ...
  if (s.isLoading) {
    return (
      <div className="space-y-3">
        ...
        <p className="pt-2 text-[13px] text-grey-2">Loading quotations and order confirmations…</p>
      </div>
    );
  }
  ...
  if (s.error) {
    return (
      <Card className="max-w-2xl p-6">
        <h1 className="text-[18px] font-bold text-navy">OCPI could not be loaded</h1>
