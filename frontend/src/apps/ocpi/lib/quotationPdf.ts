...
import { DOLLAR_CLAUSE } from "./fieldSpec";
import { fmtDealValue } from "./format";
...
function sectionRows(d: OcpiDeal, machine?: OcpiMachine): { title: string; rows: Row[] }[] {
  ...
  return [
    ...
    },
    {
      ...
      rows: [
        { label: "Machine Value", value: fmtDealValue(d.dealValueAmount, d.dealValueCurrency), wide: true },
        { label: "Term of Payment", value: d.paymentType === "advance" ? "Any Advance" : d.paymentType === "credit" ? "On Credit" : "" },
        { label: "Machine Delivery Date", value: dmy(d.deliveryDate) },
        { label: "Payment Terms", value: d.paymentTerms ?? "", wide: true },
        { label: "Term of Delivery", value: transport, wide: true },
      ],
    },
    {
      title: "D.  Remarks / Additional Information",
      rows: [{ label: "Remarks", value: d.remarks ?? "", wide: true }],
    },
  ];
}
...
export async function buildQuotationPdf(input: QuotationDocInput): Promise<jsPDF> {
  ...
  text(pdf, "NEW MACHINE QUOTATION", left + cw / 2, y + 17.5, {
  ...
  // ── Standing terms ───────────────────────────────────────────────────────
  const termLines = wrapText(pdf, DOLLAR_CLAUSE, cw, 8);
...
export function quotationFileName(deal: OcpiDeal, versionNo: number): string {
  ...
  return versionNo > 1 ? `${base} Rev ${versionNo - 1}.pdf` : `${base}.pdf`;
}
