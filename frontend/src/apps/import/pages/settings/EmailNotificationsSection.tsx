...
import { fetchEmailModuleEnabled, setEmailModuleEnabled, IMPORT_MODULE_ID } from "../../data/emailSettings";

const QK = ["emailModuleSetting", IMPORT_MODULE_ID];

/**
 * Admin: the per-module email ON/OFF switch for Import. Email only flows when
...
export default function EmailNotificationsSection() {
  ...
  const { data, isLoading } = useQuery({ queryKey: QK, queryFn: () => fetchEmailModuleEnabled(IMPORT_MODULE_ID) });
  ...
  const mut = useMutation({
    mutationFn: (v: boolean) => setEmailModuleEnabled(IMPORT_MODULE_ID, v),
    onSuccess: async () => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-[12.5px] text-grey-2 mt-1 leading-relaxed">
            When on, each Import step also emails the next responsible person(s) — the same people who get the
            in-app alert (requisition → approver, approved → PO, PO shared → collect PI, and so on). Emails are
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none">
          ...
          />
          <span>
            <span className="block text-[13.5px] font-medium text-navy">Send email notifications for Import</span>
