...
/**
 * An entry's due date with a Delayed / Today / Tomorrow chip, coloured by the
 * Control Center's four-way bucket. Lifted out of `monitoring/ControlCenter.tsx`
 * so the home dashboard's "Needs attention" list renders due dates identically.
 * Takes an already-computed `dueIso` from `lib/queues.ts`, so it can never
 * disagree with the count that put the row in the list.
 */
