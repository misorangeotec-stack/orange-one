...
export default function PoList() {
  ...
  const columns: QueueColumn<PurchaseOrder>[] = [
    ...
    { key: "shipment", header: "Shipment", cell: (p) => shipmentLabel(p.shipmentType), sortValue: (p) => shipmentLabel(p.shipmentType), filter: { kind: "select", get: (p) => shipmentLabel(p.shipmentType) }, tdClassName: "whitespace-nowrap" },
    ...
    <div className="space-y-5">
      ...
      <Card className="p-4">
        <QueueTable
          ...
          actions={(p) => (
            <Link to={`/import/pos/${p.id}`} className="text-[12.5px] font-semibold text-orange hover:underline">View</Link>
          )}
        />
      </Card>
    </div>
