...
/**
 * Every file attached at sourcing on a request LINE, as one row of links.
 *
 * ⚠ THIS IS THE READ-ONLY HALF OF THE FEATURE, AND IT MUST SIT IN `Modal`'s
 *   `readOnlyHeader` (or outside the body in edit mode). Each link mints a
 *   short-lived signed URL on click, so it has to be a button, and `Modal` puts
 *   its body inside a disabled `<fieldset>` in read-only mode — a button in the
 *   body comes up inert. Same rule, and the same reason, as `PoRefDocs`.
 *
