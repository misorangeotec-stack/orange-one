...
import type { SourcingDocInput } from "../data/importWrites";
 ...
 *
 * ⚠ THIS IS THE IMPORT COPY, AND IT KEYS ON A REQUEST LINE. Import sources one
 *   line at a time (`fms_import_save_sourcing`), where Domestic sources a whole
 *   requisition. The two files are otherwise the same by design and by the
 *   convention this repo already follows for `SignedDocCapture` /
 *   `ReceiverCopyCapture`: a component reaching across into another app's folder
 *   is how two modules end up sharing a bug.
 *
...
/** A file already filed against this request line. */
...
export async function uploadSourcingFiles(
  ...
  files: SourcingFile[],
  upload: (requestItemId: string, file: File) => Promise<SourcingDocInput>,
  ...
  for (let i = 0; i < next.length; i++) {
    ...
    const doc = await upload(requestItemId, f.file);
