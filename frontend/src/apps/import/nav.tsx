...
/** Inline SVG icon set for the import sidebar. */
const ic = {
  dashboard: (
...
/**
 * Builds the import sidebar nav. Grows as the workflow phases land.
 *
...
export function buildImportNav(opts: {
  ...
  const stepQueues: Array<{ show: boolean; label: string; to: string; icon: JSX.Element }> = [
    ...
    { show: opts.canCollectPi, label: "Collect PI", to: `${B}/queues/collect-pi`, icon: ic.inbox },
  ...
  if (opts.canMonitor) {
    nav.push({ label: `${appName("import")} Control Center`, to: `${B}/monitoring`, icon: ic.monitor, section: "Administration" });
  }
  if (opts.canManageMasters) {
    nav.push(
