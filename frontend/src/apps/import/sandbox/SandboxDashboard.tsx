...
export default function SandboxDashboard() {
  ...
  if (s.isProcessCoordinator) cards.push({ label: `${appName("import")} Control Center`, count: openPos.length + s.sourcingQueue.length + s.approvalRequestQueue.length + s.poRequestQueue.length, to: `${B}/monitoring` });
