...
export default function UserForm() {
  ...
  const { profiles, departments, profileById, addUser, updateUser } = useTaskStore();
  ...
  const submit = (e: React.FormEvent) => {
    ...
    navigate("/task-management/setup/users");
  };

  return (
    <div className="max-w-2xl space-y-5">
      <button onClick={() => navigate("/task-management/setup/users")} className="text-[13px] text-grey hover:text-orange font-medium inline-flex items-center gap-1">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <FieldLabel label="Department">
              <Combobox value={departmentId} onChange={setDepartmentId} placeholder="— None —" options={[{ value: "", label: "— None —" }, ...departments.map((d) => ({ value: d.id, label: d.name }))]} />
            </FieldLabel>
          </div>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2">
            <Button variant="ghost" onClick={() => navigate("/task-management/setup/users")}>Cancel</Button>
