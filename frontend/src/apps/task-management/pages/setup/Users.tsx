...
export default function Users() {
  const { profiles, departments, departmentById, profileById, deleteUser } = useTaskStore();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button size="sm" onClick={() => navigate("/task-management/setup/users/new")}>
      ...
      <Card className="overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <EmptyState title="No users found" message="Try different filters, or add a user." actionLabel="Add User" actionTo="/task-management/setup/users/new" />
        ) : (
          <ul className="divide-y divide-line">
            {filtered.map((u) => (
              <li key={u.id} className="flex items-center gap-3 px-4 py-3.5">
                ...
                <Link to={`/task-management/setup/users/${u.id}/edit`} className="text-grey-2 hover:text-orange transition p-1 shrink-0" title="Edit">
