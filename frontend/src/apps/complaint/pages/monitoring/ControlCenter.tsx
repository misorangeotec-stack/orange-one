...
/**
 * The Complaint Control Center — the SAME queue entries every page counts, bucketed
 * by due date. Built through `snapshotFrom(store.queueEntries, STEPS, STAGES)`,
 * exactly as the cross-FMS scoreboard adapter does, so the two can never disagree.
 *
 * ⚠ THE APPROVAL STAGE IS USUALLY EMPTY, and that is correct rather than a fault:
 *   only a credit note above the Setup threshold ever routes there. An empty
 *   stage still renders, because "nothing is stuck here" and "this step does not
 *   exist" have to look different to a coordinator.
 */
export default function ControlCenter() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-[22px] font-bold text-navy">{appName("complaint")} Control Center</h1>
        <p className="text-[13.5px] text-grey-2 mt-1">Open work across the whole complaint process, by how close it is to its due date.</p>
      </div>
