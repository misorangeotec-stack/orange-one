 ...
 * anchor step completes. `raise` has no queue, so it is hidden.
 *
 * ⚠ ONE EXCEPTION IS NOT SETTABLE HERE, deliberately: once a target date has been
 *   PROMISED TO THE PARTY at acknowledge, that date — not this number — is when
 *   the resolution is due (lib/queues.ts). The note below says so, because an
 *   admin who set "5 days" and then saw a different date on a queue would
 *   reasonably think the setting was broken.
 */
  ...
  return (
    <Card className="p-5 space-y-4 max-w-xl">
      <p className="text-[12.5px] text-grey">
        How many working days (Mon–Sat) each step gets after the previous one completes. A complaint
        past its due date shows red in the queues and the Control Center.
      </p>
          ...
          <div key={st.key} className="flex items-center justify-between gap-4">
            <div>
              ...
              <div className="text-[11px] text-grey-2">
                After {stepByKey(s.stepSla[st.key]?.anchor ?? "raise")?.short ?? "Raised"}
              </div>
            </div>
