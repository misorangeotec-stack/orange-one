 ...
 * and can hold a complaint. Stored in fms_complaint_config under
...
export default function CoordinatorsSection() {
  ...
  const save = async () => {
    ...
    setSaved(false);
    try {
      await s.saveProcessCoordinators(picked);
      setSaved(true);
    } catch (e) {
  ...
  return (
    <Card className="p-5 space-y-4 max-w-xl">
      <p className="text-[12.5px] text-grey">
        Coordinators oversee the whole complaint process — they can act on any step, hold a
        complaint, and open the Control Center.
      </p>
      <MultiSelect
        values={picked}
        onChange={(v) => {
