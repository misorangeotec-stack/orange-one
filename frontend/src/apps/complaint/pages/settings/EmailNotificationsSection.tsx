...
const QK = ["emailModuleSetting", COMPLAINT_MODULE_ID];

/**
 * Admin: the per-module email ON/OFF switch. Email only flows when this is on; the
 * in-app bell is never affected. Backed by the shared email_module_settings table
 * + the set_email_module_enabled RPC.
 */
export default function EmailNotificationsSection() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: QK,
    queryFn: () => fetchEmailModuleEnabled(COMPLAINT_MODULE_ID),
  });
  ...
  const mut = useMutation({
    mutationFn: (v: boolean) => setEmailModuleEnabled(COMPLAINT_MODULE_ID, v),
    onSuccess: async () => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-[12.5px] text-grey-2 mt-1 leading-relaxed">
            When on, each step also emails the next responsible person(s) — the same people who get
            the in-app alert: a raised complaint goes to whoever acknowledges, an acknowledgement to
            the named investigator, an investigation to the named CAPA owner, and so on back to the
            raiser at close. Emails are sent from support@orangeotec.com. In-app alerts are
            unaffected.
          </p>
        </div>
        ...
        <label className="flex items-start gap-3 cursor-pointer select-none">
          ...
          <span>
            <span className="block text-[13.5px] font-medium text-navy">
              Send email notifications for Complaints
            </span>
