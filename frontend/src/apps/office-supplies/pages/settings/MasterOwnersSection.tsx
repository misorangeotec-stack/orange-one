...
import { SUPPLY_MASTER_TYPES, type SupplyMasterType } from "../../types";

/**
 * Master Owners (admin only). All five masters are ownable; only Items and Service types
 * are requestable — the rest are structural and edited directly on the Masters page.
 */
export default function MasterOwnersSection() {
  const s = useSuppliesStore();
  const [editing, setEditing] = useState<SupplyMasterType | null>(null);
  ...
  const requestable = (mt: SupplyMasterType) => mt === "item" || mt === "service_type";

  return (
    <Card className="overflow-hidden">
      <div className="px-4 pt-4 pb-1">
        <p className="text-[12.5px] text-grey">
          Each master's owner reviews the "request a new entry" submissions and can approve or reject them. Leave a master
          unassigned and its requests go to the admins.
        </p>
      </div>
      <table className="w-full text-[13.5px]">
        <thead>
          <tr className="text-left text-grey-2 border-b border-line">
            ...
            <th className="font-medium px-4 py-3">Pending</th>
          </tr>
        </thead>
        <tbody>
          {SUPPLY_MASTER_TYPES.map((mt) => {
            ...
            return (
              <tr key={mt.value} className="border-b border-line/70 last:border-0 hover:bg-page/60">
                ...
                <td className="px-4 py-3">
                  {requestable(mt.value) ? <span className="text-navy">Yes</span> : <span className="text-grey-2">No — edited on Masters</span>}
                </td>
                <td className="px-4 py-3">{pending ? <span className="text-orange font-semibold">{pending}</span> : <span className="text-grey-2">—</span>}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <Modal
        ...
        title={`Owners — ${SUPPLY_MASTER_TYPES.find((m) => m.value === editing)?.plural ?? ""}`}
