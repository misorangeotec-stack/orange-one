...
import { fetchEmailModuleEnabled, setEmailModuleEnabled, OFFICE_SUPPLIES_MODULE_ID } from "../../data/emailSettings";
...
export default function EmailNotificationsSection() {
  ...
  const { data, isLoading } = useQuery({ queryKey: QK, queryFn: () => fetchEmailModuleEnabled(OFFICE_SUPPLIES_MODULE_ID) });
  ...
  const mut = useMutation({
    mutationFn: (v: boolean) => setEmailModuleEnabled(OFFICE_SUPPLIES_MODULE_ID, v),
    onSuccess: async () => {
