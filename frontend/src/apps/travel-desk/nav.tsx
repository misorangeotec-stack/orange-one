...
const ic = {
  ...
  trips: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="16" rx="2" /><path d="M7 9h10M7 13h10M7 17h6" /></svg>),
  ...
  rates: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 3h12l3 6-9 12L3 9z" /><path d="M9 9h6" /></svg>),
 ...
 *
 * Kept SHORT and human — a service engineer reads
 * "/travel-desk/queues/my-claim", not the step key. The keys stay the database's
 * business.
 */
export const QUEUE_PATH: Record<QueueStep, string> = {
  manager_approval: "approve-trip",
  director_approval: "director-approval",
  advance: "advance",
  booking: "booking",
  claim: "claims-due",
  claim_review: "approve-claim",
  finance_review: "finance-check",
  settlement: "settlement",
};

export function buildTravelNav(opts: {
  isAdmin: boolean;
  canRaise: boolean;
  canSetup: boolean;
  queues: Record<QueueStep, boolean>;
  /** Any master owned, or admin — the Masters screen and the rate cards. */
  ...
  const nav: NavItem[] = [
    ...
    { label: "All Trips", to: `${B}/trips`, icon: ic.trips },
    { label: "My Trips", to: `${B}/mine`, icon: ic.mine },
    // Drafts get their own destination rather than a filter on the list: a draft
    // is private to its author and is the one thing a traveller comes back to
    // finish, so burying it behind a filter is how it gets forgotten.
    { label: "Drafts", to: `${B}/drafts`, icon: ic.draft },
  ];

  if (opts.canRaise) {
    nav.push({ label: "New Trip Request", to: `${B}/new`, icon: ic.add, section: "Actions" });
  }
  ...
  for (const st of STEPS.filter((x) => !x.noQueue)) {
    ...
    // Cancellations are the booking step's other job, and they need their own
    // destination: unwinding a booking is the opposite work from making one, and
    // it is the urgent one because a refund window closes.
    if (step === "booking") {
      nav.push({ label: "Cancellations", to: `${B}/queues/cancellations`, icon: ic.step });
    }
  }

  // The pipeline sits with the reports rather than in Workspace: it is the
  // coordinator's board, not a personal list, and putting it above "All Trips"
  // would make it the first thing a traveller lands on.
  nav.push(
    ...
    { label: "Cancelled Travel", to: `${B}/reports/cancelled`, icon: ic.report },
    { label: "Policy Exceptions", to: `${B}/reports/exceptions`, icon: ic.report },
    { label: "GST Input Credit", to: `${B}/reports/gst-itc`, icon: ic.report },
    { label: "Spend Summary", to: `${B}/reports/spend`, icon: ic.report },
    { label: "Desk Performance", to: `${B}/reports/performance`, icon: ic.report },
  );

  // Master requests is NOT admin-only: anyone with edit access raises one (a
  // city that is not on the list, a hotel nobody has used before) and wants to
  // see what happened to it. The badge counts what is still waiting.
  ...
  if (opts.canManageAnyMaster) {
    nav.push(
      ...
      // Rate cards get their own destination rather than a Masters tab: a rate
      // card is a versioned document with an effective date and a sign-off, not
      // a list of names, and it is the one screen a Director is asked to
      // approve.
      { label: "Rate Cards", to: `${B}/rate-cards`, icon: ic.rates },
    );
  }
